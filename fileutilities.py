        ...
        if line[0] == '#' or line == "\n":
            ...
            for c in cols:
                ...
                if len(v) == 1:
                    ...
                    try:
                        expandedcols.extend(range(int(v[0]), int(v[1]) + 1))
                    except TypeError:
                        expandedcols.extend(range(labels.index(v[0]), labels.index(v[1]) + 1))
...
def get_random_columns(flist, colSep=["\t"], k=1, header=False, index=None, merge=True):
    ...
    for f, (myfile, myalias) in flist.enum():
        ...
        if index is not None:
            ...
            # range() is right-open. 
            cols.extend(random.sample(range(0, fieldNums[f]-1), k))  
        else:
            cols = random.sample(range(0,fieldNums[f]), k)
...
def main(args):
    ...
    *succeeds* the relevant output, rather than precede it. This serves as 
    ...
    # General tasks.
    parser.add_argument('--probe', type=str, choices=_funcDispatch.keys(),
    ...
    # LOOP arbitrary command. -------------------------------------------------
        ...
        if params.loop[0] == 'R':
            ...
            for t in targets:   # Look for multiple ranges.
                v = t.split(":")
                if len(v) > 1:
                    myrange.extend(range(int(v[0]), int(v[1]) + 1))
                else:
