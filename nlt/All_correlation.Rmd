...
nltmeta <- fread("~/Camilla/Neurolentech/SampleInfo_Batch_1,2,3.txt")
...
BSP <- fread("~/Camilla/Brainspan/expression_matrix.csv")
bspc <- fread("~/Camilla/Brainspan/columns_metadata.csv")
bspr <- fread("~/Camilla/Brainspan/rows_metadata.csv")
...
<!-- This section was added retro-actively, after seeing the correlation results. Due to size/resolution, the heatmaps are sent directly to PDF. -->
...
bspc[, age_value := vapply(strsplit(age, "\\s"), function(x){ as.numeric(x[1]) }, numeric(1)) ]
bspc[, age_unit := factor(vapply(strsplit(age, "\\s"), function(x){ x[2] }, character(1)), levels=c('pcw', 'mos', 'yrs'))]
...
bspc[, new_column := gsub("\\W", "_", paste(age_unit, age_value, new_tissue, gender, column_num, sep="_"))]

# Apply the new names
setnames(BSP, c("row_num", bspc$new_column))

# Manual order for easier interpretation
setorder(bspc, age_unit, age_value, new_tissue, structure_acronym)
BSP <- BSP[, c(0, bspc$column_num) + 1, with = FALSE]

bspc[, age := factor(age, levels=unique(age))]
```
...
write(unique(bspr[, ensembl_gene_id]), file="~/Camilla/Brainspan/ensembl_ids.txt")
...
ensembl_hist38 <- rbind(fread("~/Camilla/Brainspan/ensembl_ids_idmapper38.txt", header = FALSE, fill = TRUE, sep = ",",
                               col.names = c("old", "new", "release", "score")) ) # short titles without spaces
```
...
setorder(BSP, row_num)
BSP[, row_num := as.character(row_num)]
setorder(bspr, row_num)
...
nltmeta[Timepoint == '6W', Timepoint := 'W6']
nltmeta[, new_column := paste(Timepoint, Batch, sBatch, Construct, Sex, col_num, sep = "_")]
...
setorder(nltmeta, Type, Timepoint, Batch, sBatch, Construct)
NLT <- NLT[, c("gene", "gene_id", nltmeta$new_column), with = FALSE]
```
...
BSP <- merge(bspr[, .(row_num, new_ensembl)], BSP, by="row_num", all.y=TRUE)
```
...
cat("Brainspan variables\n")
print(table(bspc[, .(structure_name, gender)]))
print(table(bspc[, .(age, gender)]))
print(table(bspc[, .(structure_name, age)]))
...
print(table(nltmeta[, .(Construct, Type)]))
print(table(nltmeta[, .(Type, Sex)]))
print(table(nltmeta[, .(Batch, Sex)]))
print(table(nltmeta[, .(Batch, Construct)]))
print(table(nltmeta[, .(Batch, sBatch)]))
print(table(nltmeta[, .(sBatch, Sex)]))
print(table(nltmeta[, .(sBatch, Construct)]))
...
pdf(file="~/Camilla/CORRELATION/All_unclustered.pdf", width = 25, height = 100)
print(all_pearson_plots$pfr)
print(all_spearman_plots$pfr)
dev.off()

pdf(file="~/Camilla/CORRELATION/All_clustered.pdf", width = 35, height = 110)
print(all_pearson_plots$pfrc)
print(all_spearman_plots$pfrc)
dev.off()
```
...
```{r all_cor_filtered}
all_pearson_filtered <- my_pairwise_external_correls(BSP, NLT, method="pearson", minMean = 50, minSingle = 100, log = FALSE)
all_pearson_plots_filtered <- plot_my_correlations(all_pearson_filtered)

all_spearman_filtered <- my_pairwise_external_correls(BSP, NLT, method="spearman", minMean = 50, minSingle = 100, log = FALSE)
all_spearman_plots_filtered <- plot_my_correlations(all_spearman_filtered)

pdf(file="~/Camilla/CORRELATION/All_filtered_log_unclustered.pdf", width = 25, height = 100)
print(all_pearson_plots_filtered$pfr)
print(all_spearman_plots_filtered$pfr)
dev.off()

pdf(file="~/Camilla/CORRELATION/All_filtered_log_clustered.pdf", width = 35, height = 110)
print(all_pearson_plots_filtered$pfrc)
print(all_spearman_plots_filtered$pfrc)
dev.off()
```
