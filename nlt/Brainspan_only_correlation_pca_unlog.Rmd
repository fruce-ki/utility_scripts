---
title: "PCA & Correlations - Brainspan"
...
bspc[, Sample := gsub("\\W", "_", paste(age_unit, age_value, new_tissue, gender, column_num, sep="_"))]
setorder(bspc, column_num) # Should already be, but I'm not leaving it to chance
setnames(BSP, c("row_num", bspc$Sample))
...
counts <- as.matrix(BSP[, 2:ncol(BSP)])
rownames(counts) <- bspr[BSP$row_num, ensembl_gene_id]

tpm <- counts
totals <- colSums(tpm)
tpm <- sweep(tpm, 2, totals, `/`) * 1e6

setorder(bspc, age_unit, age_value, new_tissue)
covars <- bspc[, .(Sample, donor_name, age_unit, age_value, gender, new_tissue, structure_name)]

params <- list(cts = "~/Camilla/CORRELATIONS/Brainspan_unlog.tsv",
...
```{r corr1, fig.height=100, fig.width=100}
...
setorder(params$covars, age_unit, age_value)
...
setorder(params$covars, new_tissue, structure_name)
corels1 <- plot_my_correlations(my_pairwise_internal_correls(params$tpm[topgenes,], params$covars$Sample, 
                                                            method="pearson", minMean = 0, minSingle = 0), # prefiltered
                                                            rds = file.path(params$RDSdir, 
                                                                            sub(".txt|.tsv", "_corrP.RDS", basename(params$cts))) )
...
setorder(params$covars, donor_name)
corels1 <- plot_my_correlations(my_pairwise_internal_correls(params$tpm[topgenes,], params$covars$Sample, 
                                                            method="pearson", minMean = 0, minSingle = 0), # prefiltered
                                                            rds = file.path(params$RDSdir, 
                                                                            sub(".txt|.tsv", "_corrP.RDS", basename(params$cts))) )
