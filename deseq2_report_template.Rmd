...
# params <- list(selfname = name, derds = ddsrds, rmrds = rpmrds, ntop = opt$ntop, pcutoff = opt$pcutoff, lfcthreshold = opt$lfcthreshold, baseDir = opt$baseDir, resultsDir = opt$resultsDir, prefix = autoname, ppen = 5, minMean=opt$minMean, minSingle=100, filterBaseMean=opt$bmF, reducedFormula = opt$reducedFormula, roundrobin = opt$all)
...
vstmat = NULL
tryCatch(vstmat <- assay(vst(deobj, blind=FALSE)),     # Variance stabisized counts,
         error = function(e) {
           print(e)
           stop("At least two samples per condition are expected, and at least 1000 expressed genes in the relevant samples.")
         }
)
...
cat(paste("Report name: ", params$selfname))
...
cat(paste0("log2FC threshold: ", params$lfcthreshold, " , (i.e. ", 2 ^ params$lfcthreshold, "-fold change)"))
...
```{r functionDE}
# DE
do_de_plots <- function(contrast, dds = deobj, rpms = rpm, ntop = params$ntop, 
                        pcutoff = params$pcutoff, lfcthreshold = params$lfcthreshold, filterBM=params$filterBaseMean, prefix=params$prefix){
  # contrast <- coeffs[1]
  ...
  ## Heuristic selection of top hits to highlight, by eucledian distance from origin.
  ...
  # Cluster the genes and plot the heatmaps
  mat <- dcast(selcnt, name ~ sample, value.var='RPM')
  rn <- mat[[1]]
  cn <- names(mat)
  mat <- as.matrix(mat[,2:length(mat), with=FALSE])
  colnames(mat) <- cn[2:length(cn)]
  rownames(mat) <- rn
  hcfit <- hclust(dist(scale(mat, center=TRUE)))
  selcnt[, name := factor(name, ordered=TRUE, levels=rn[hcfit$order])]
  selcnt[, sample := factor(sample, ordered=TRUE, levels=rownames(coldata))]
  # Keep the same gene order
  selvst[, name := factor(name, ordered=TRUE, levels=rn[hcfit$order])]
  selvst[, sample := factor(sample, ordered=TRUE, levels=rownames(coldata))]
  
  ph1 <- ggplot(selcnt, aes(x=sample, y=name, fill=log10(RPM))) +
            geom_tile() +
            facet_grid(. ~ type) +
  ...
  ph2 <- ggplot(selvst, aes(x=sample, y=name, fill=VST_count)) +
            geom_tile() +
            facet_grid(. ~ type) +
            scale_fill_gradient(high="white", low="darkgreen", na.value = "black") +
            labs(title=contrast) +
  ...
  outtab <-merge(outtab,
                 data.table(name=rownames(rpms),
                            refMeanRPM=rowMeans(rpms[, refs]),
                            treatMeanRPM=rowMeans(rpms[, treats]) ),
                 all=TRUE, by="name")
  ...
  zeros <- min(outtab[refMeanRPM > 0, refMeanRPM], outtab[treatMeanRPM > 0, treatMeanRPM]) / 10
  ...
  pe1 <- ggplot(NULL, aes(x=refMeanRPM + zeros, y=treatMeanRPM + zeros, label=name, colour=category)) +
      ...
      labs(title=contrast, x='reference Mean RPM', y='treatment Mean RPM') +
      theme(panel.grid.minor = element_blank())
  
  pe1ly <- ggplot(NULL, aes(x=refMeanRPM + zeros, y=treatMeanRPM, label=name, colour=category)) +
      geom_abline(intercept = 0, slope = 1, colour='black') +
      geom_point(data=outtab[category == highlight[3], ], alpha=0.7) +
      geom_point(data=outtab[category == highlight[4], ]) +
      scale_x_log10() + 
  ...
  pe2 <- ggplot(NULL, aes(x=refMeanRPM + zeros, y=refStDev, label=name, colour=category)) +
      ...
      scale_colour_manual(values=hlcol) +
      annotation_logticks(base=10, sides='lrtb') +
      labs(title=contrast, x='reference Mean RPM', y='reference St Dev') +
      theme(panel.grid.minor = element_blank()) 
  
  pe3 <- ggplot(NULL, aes(x=treatMeanRPM, y=treatStDev, label=name, colour=category)) +
      geom_point(data=outtab[category == highlight[1], ], alpha=0.4, size=0.8) +
      geom_point(data=outtab[category == highlight[2], ], alpha=0.7, size=0.8) +
      geom_point(data=outtab[category == highlight[3], ], alpha=0.7) +
      geom_point(data=outtab[category == highlight[4], ]) +
      ...
      scale_colour_manual(values=hlcol) +
      annotation_logticks(base=10, sides='lrtb') +
      labs(title=contrast, x='treatment Mean RPM', y='treatment St Dev') +
      theme(panel.grid.minor = element_blank()) 
  ...
  setnames(outtab,
           c(names(outtab)[1],              # keep uniform name across tables for the joining key
             paste(prefix,                  # make the other columns unique
...
Correlations of the samples, based on the RPM-scaled counts and the Variance-Stabilisation-Transformed counts. The RPMs ensure that library size does not influence correlation. VST counts adjust for the inflated variance of low count genes. 

Pearson correlation uses the actual values and measures the linear relationsip between samples (ie that gene expressions between two samples differ only by a constant factor applied to all genes). Spearman correlation instead uses the rank of the features's expression within the transcriptome, and is not sensitive to the exact expression values.

The first plot in each subsection is scaled to the full range of correlation values, and gives an objective overview of how good correlations are (or aren't). The samples are ordered on the axes as per the order provided.
The second plot is scaled to the range of values actually obtained for this dataset and the samples are ordered by hierarchical clustering. This amplifies contrast and structure. Samples belonging to the same condition are expected to cluster next to each other.

## RPM counts

```{r correlations1, fig.height=6, fig.width=9}
corels1p <- my_pairwise_internal_corels(rpm, colnames(rpm), method="pearson", 
                                       minMean=params$minMean, minSingle=params$minSingle)
corels1s <- my_pairwise_internal_corels(rpm, colnames(rpm), method="spearman", 
                                       minMean=params$minMean, minSingle=params$minSingle)
...
```{r correlations2, fig.height=9, fig.width=9}
...
corels2p <- my_pairwise_internal_corels(vstmat, colnames(rpm), method="pearson", 
                                       minMean=params$minMean, minSingle=params$minSingle)
corels2s <- my_pairwise_internal_corels(vstmat, colnames(rpm), method="spearman", 
                                       minMean=params$minMean, minSingle=params$minSingle)
...
The first subsection below contains static volcano and MA plots of *all the genes*.
...
The highlighted red triangles are the up to `r params$ntop` top hits out of all the points that meet the thresholds. This number limit is arbitrary. "Top-ness" of hits is ranked roughly based on their "distance" from the origin of the axes in the volcano plots and is not a substitute for judgement based on experience.
...
For each comparison the following are shown:

* A list of highlighted "top" hit genes. This selection is for quick first view purposes. An effort is made to prioritize highest fold-change lowest p-value genes in this selection, but it is not a definitive selection of "best" candidates. The number of genes highlighted is arbitrary and reflects practical considerations of how many labels can reasonably be displayed in a scatter plot of a given size and resolution without overcrowding. 
* Volcano plot with the "top" hits highlighted.
* MA plot with the "top" hits highlighted. This is a way to check how much support there is across the samples for the fold-change of a gene. MA plots also show if there is a global skew in fold changes, though this is only the case when DESeq2 is explicitly prevented from scaling the data.
* Covariation scatter plot of the mean RPMs for the two conditions, with all hits highlighted. Mean RPMs are increased by 1/10th the value of the smallest non-zero RPM value in the dataset, in order to allow genes with a zero mean value in one condition to be displayed on log-scale axes while remaining distinct from non-zero values and without significant impact on non-zero values.

Fold-change shrinking is a procedure that aims to counter the inflated fold-changes that are produced by low-counts, and can be considered an alternative/orthogonal method to the p-value. "Top" hits selected here have shrunken LFCs that still satisfy the LFC threshold. Therefore, if you see hits in the standard plots below that seem like they should have been marked as "top" but were skipped, it is likely because they have low raw count and their shrunken LFC fell below the threshold.

The plots below, as well as additional plots including versions with shrunken LFCs, are available in the accompanying PDF file.
...
for (res in resall) {
  fwrite(res$outtab,
         file = file.path(params$baseDir,
                          params$resultsDir,
                          paste(params$prefix, 
                                paste(sub('[^A-Za-z0-9_\\-.+~]', '-', design(deobj), perl=TRUE), collapse=''), # redundant but helps group table files with the report file.
                                res$contrast, 
                                'deseq2.tsv', 
                                sep='.')),
...
pdf(file.path(params$baseDir, 
              params$resultsDir, paste(params$prefix, 
                                       paste(sub('[^A-Za-z0-9_\\-.+~]', '-', design(deobj), perl=TRUE), collapse=''), 
                                       'deseq2.pdf', sep='.')),
...
# Correlation plots
print(corels1p[['frnc']] + labs(substitle='RPM'))
print(corels1s[['drc']] + labs(substitle='RPM'))
