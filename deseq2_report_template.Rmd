...
```{r functionDE}
# DE
do_de_plots <- function(contrast, dds = deobj, rpms = rpm, ntop = params$ntop, addPrefix = params$longlabel,
  ...
  cnt <- merge( data.table(name = rownames(counts(dds)),
                ...
                all=TRUE, by="name", suffixes = c('.Count', '.RPM') )
  ...
  outtab[, stat := NULL]
  outtab[, mlog10p := NULL]
  outtab[, mlog10p.shrink:= NULL]
  outtab[, padj.shrink := NULL]
  outtab[, lfcSE.shrink := NULL]
  ...
  if (addPrefix) {
    ...
    # Add as suffix insted. Makes life easier in spotfire
    setnames(outtab, c(names(outtab)[1],      # keep uniform name across tables for the joining key
...
```{r csv}
for (res in resall) {
  res$outtab[, meetthresh := NULL]
  res$outtab[, istop := NULL]
  fwrite(res$outtab,
