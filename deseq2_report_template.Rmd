...
# params <- list(selfname = paste0(paste(autoname, gsub(' |~', '', opt$designFormula), sep='_'), '_deseq2.html'), derds = ddsrds, rmrds = rpmrds, ntop = opt$ntop, pcutoff = opt$pcutoff, lfcthreshold = opt$lfcthreshold, baseDir = opt$baseDir, resultsDir = opt$resultsDir, prefix = autoname, ppen = 5, minL=0.75, minCount=opt$minCount, filterBaseMean=opt$bmF, reducedFormula = opt$reducedFormula, roundrobin = opt$all)
...
tryCatch(vstmat <- assay(vst(deobj, blind=FALSE)),     # Variance stabisized counts,
         error = function(e) {
           stop("WARNING: Only one replicate per condition is available! DESeq2 no longer allows or supports estimating variances by pooling samples across conditions.")
         }
)

cD <- colData(deobj)[, 1:length(colData(deobj)), drop=FALSE]  # before model building, there in no size factor as last column

cat(paste("Report: ", sub('.html', '', params$selfname, fixed=TRUE)))
...
cat(paste("Max number of top hits to label in plots:", nflag))
...
do_de_plots <- function(contrast, dds = deobj, rpms = rpm, ntop = params$ntop, ppen = params$ppen, 
  ...
  # Heuristic selection, by eukledian distance, of top hits to highlight when there are more hits than the given limit
  ...
  lfc[name %in% shrink[(istop), name], istop := TRUE]
  ...
  # Choose subset of top genes to flag, by order of shrunken LFC
  if (any(shrink$istop)){
    flags <- head(shrink[(istop), ][order(dist, decreasing=TRUE)][, name], n=nflag)
  } else {
    flags <- head(shrink[, ][order(abs(log2FoldChange), decreasing=TRUE)][, name], n=5)
  }
  
  # Combine shrunken and non-shrunk LFC tables for output.
  outtab <- rbind(lfc, shrink, fill=TRUE)
  outtab[, addflag := FALSE]
  outtab[, addflag := name %in% flags]
  outtab[(addflag), flag := name]
  ...
  pv1 <- ggplot(NULL, 
                aes(x=log2FoldChange, y=mlog10p, 
                    ...
                    label=flag)) + 
            facet_grid( . ~ type) +
            geom_point(data=outtab, size=rel(0.8), alpha=0.5) +
            ...
            geom_text_repel(data=outtab[(addflag),], size=rel(2.5), segment.size = 0.3, min.segment.length=0, arrow = arrow(length = unit(0.008, "npc")), max.overlaps=1e4, force=1, force_pull=1) +
  ...
  # Add shrunken LFC column to normal LFC table. 
  # Previously the tables were joined in long format, which is great for facetting in plots. For human use, wide format is preferable.
  # dcast is used here as a glorified rename, but without changing the original (it made more sense when this block was located earlier in the function)
...
* Red triangles for the top `r params$ntop` (or fewer) hits.
* Black tiles for everything else. Density of points indicated by fade/transparency of the tiles.
* Blue for genes that were not assigned a  p-value (filtered out before testing).
* Labels added only to a subset of the top genes by order of "shrunken" Log2FC, to avoid excessive clutter.

```{r expov, warning=FALSE, fig.height=8, fig.width=12}
pexp <- lapply (resall, function(res) {
  ...
  # Expression and variance densities
  ###################################
  ...
  DT[(istop), colour := 'top']
  DT[, addflag := FALSE]
  flags <- head(DT[(istop), ][order(shrunken_L2FC, decreasing=TRUE), name], n=nflag)
  DT[name %in% flags, addflag := TRUE]
  DT[(addflag), flag := name]
  ...
  p1 <- ggplot(NULL, aes(x=refMeanRPM + zeros, y=treatMeanRPM + zeros, colour=colour, label=flag)) +
      ...
      geom_abline(intercept = 0, slope = 1, colour='grey75', alpha=0.3) +
      geom_point(data=DT[(istop), ], shape=17, size=1.8) +
      geom_label_repel(data=DT[(istop), ], size=rel(2.5), segment.size = 0.3, min.segment.length=0, arrow = arrow(length = unit(0.008, "npc")), max.overlaps=1e4, force=10, force_pull=5) +
  ...
  if (!all( is.na(DT$refStDev) )) {
    p2 <- ggplot(NULL, aes(x=refMeanRPM + zeros, y=refStDev / (refMeanRPM + zeros), colour=colour, label=flag)) +
        ...
        geom_point(data=DT[colour=='pNA', ], shape=20, alpha=0.2, size=0.8) +
        geom_point(data=DT[(istop), ], shape=17, size=1.8) +
        geom_label_repel(data=DT[(istop), ], size=rel(2.5), segment.size = 0.3, min.segment.length=0, arrow = arrow(length = unit(0.008, "npc")), max.overlaps=1e4, force=10) +
      scale_colour_manual(values=c(general='black', top='red', pNA='steelblue')) +  
        scale_fill_gradient2(high='black') +
        scale_x_log10() + 
        scale_y_log10() + 
  ...
  if (!all( is.na(DT$treatStDev) )) {
    p3 <- ggplot(NULL, aes(x=treatMeanRPM + zeros, y=treatStDev / (treatMeanRPM + zeros), colour=colour, label=flag)) +
      # geom_point(data=DT[colour=='black', ], shape=16, alpha=0.2, size=0.8) +
      geom_hex(data=DT[colour=='general', ], bins=50, size=0.5, colour='transparent') +
      geom_point(data=DT[colour=='pNA', ], shape=20, alpha=0.2, size=0.8) +
      geom_point(data=DT[(istop), ], shape=17, size=1.8) +
  ...
  print( p1 )
  print( (p2 + theme(legend.position="none")) | (p3 + theme(legend.position="none")) )

  # Static Volcano and MA
  print(res[["pv1"]])
  print(res[["pma1"]])
  
  return(p1 | (p2 / p3)) # for the PDF
})
```
...
## Expression heatmap of all top hits from all comparisons
...
# Correlation plots
print(corels1[['sfrnc']] + labs(substitle='raw counts'))
print(corels1[['tdrc']] + labs(substitle='raw counts'))
print(corels2[['sfrnc']] + labs(substitle='RPM'))
print(corels2[['tdrc']] + labs(substitle='RPM'))
print(corels3[['sfrnc']] + labs(substitle='VST counts'))
print(corels3[['tdrc']] + labs(substitle='VST counts'))
