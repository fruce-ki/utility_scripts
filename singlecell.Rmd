...
# plot_grid_pcs(so, comps= custom_pcs[c(1:5)])
plot_grid_pcs(so, comps= c(6, 8:11))                        # USER-INPUT
```
...
fwrite(dcast(as.data.table(table(so@meta.data[, c("Source", "seurat_clusters")])),
			 ...
			 file=file.path(outdir, paste0(projectname, '_clustertallies_customdim.tsv')),
...
```{r clustmark, fig.height=10, fig.width=10}
do_de(so, DEtabled=DEtabled, lfcthresh=lfcthresh, pctexpthresh=pctexpthresh, diffpct=diffpct, suffix='customdim')
```
