...
knitr::opts_chunk$set(echo = TRUE, fig.path='SC_analysis_figs/', dev=c('png', 'pdf'))
```
...
```{r info}
datadirs <- c(control = NULL,
							treatment = NULL)

projectname = 'SC_analysis'
outdir <- '~/'

# Markers of interest        # USER-INPUT

# Must express.
positivefilter <- list(control=c(NULL),
											 treatment=c(NULL))

# Must NOT express.
negativefilter <- list(control=NA_character_,
...
# USER_INPUT
cccore <- read.table('~/cellcycle.txt', stringsAsFactors=FALSE)[[1]]
...
# Gene signatures https://doi.org/10.1080/2162402x.2020.1737369 (excluding cell cycle)
# USER_INPUT
signatures <- NULL
...
plot_qc_detection <- function(so, detectionthresh=0){
	...
	p1 <- ggplot(detectionstats, aes(x=mean.readCount, fill=Source)) +
		...
		labs(titlee = "Distribution of genes by their mean detection level") +
		theme_bw() + 
		labs(x=NULL, y="Genes") +
	...
	p2 <- ggplot(detectionstats, aes(x=mean.readCount, y=pct.Cells, colour=Source)) +
		...
		scale_x_log10() +
		annotation_logticks(sides="b") +
		labs(title="% of cells in which a gene is detected", subtitle="as function of the gene's mean detection level") +
		theme_bw() + 
		theme(legend.position="none",
...
varscanplot <- function(v, candidates){
	p1 <- ggplot(v) +
		geom_vline(xintercept=candidates['ksum2'], linetype='dotted', colour='forestgreen', alpha=0.6) +
		geom_vline(xintercept=candidates['kelbow'], linetype='dotted', colour='blue', alpha=0.6) +
		geom_vline(xintercept=candidates['mindiff2'], linetype='dashed', colour='black', alpha=0.6) +
	...
	p2 <- ggplot(v) +
		geom_vline(xintercept=candidates['maxelbow'], linetype='dashed', colour='orange', alpha=0.6) +
		geom_vline(xintercept=candidates['kover1'], linetype='dotted', colour='purple', alpha=0.6) +
		geom_vline(xintercept=candidates['ksum80'], linetype='dotted', colour='magenta', alpha=0.6) +
		geom_vline(xintercept=candidates['minover1'], linetype='dashed', colour='purple', alpha=0.6) +
		geom_vline(xintercept=candidates['minsum80'], linetype='dashed', colour='magenta', alpha=0.6) +
...
varscan <- function(so, n, exclude=NULL, varcutoff=80) {
	...
	candidates <- c(minover1 = n[ min(which(v$over_1 == min(v$over_1))) ],
								...
								maxelbow = n[which(v$elbow == max(v$elbow))][1],
...
plot_cell_density <- function (so, dims = c(1, 2), cells = row.names(so@meta.data),
					reduction = 'umap', split.by = 'Source', relative=FALSE, binnum=50, withumap=FALSE) 
{
	if (length(x = dims) != 2) {
		stop("'dims' must be a two-length vector")
	}
	embed <- as.data.frame(Embeddings(object = so[[reduction]]))  #can't be as.data.table : need the rownames
	data <- embed[cells, dims]  
	data <- cbind(as.data.table(data), data.table(Cells = rownames(data))) #gymnastics for cbind to behavee correctly even with nrow==1
	data[, Cells := as.factor(Cells)]
	data[ , split.by := so@meta.data[cells, split.by]]
	setnames(data, c(names(data)[-length(data)], split.by))
	dims <- paste0(Key(object = so[[reduction]]), dims)
	
	if(nrow(data) > 1){
		# Keep contant ranges across independent plots of subsets of the data
		xmin <- min(embed[, dims[1]])
		xmax <- max(embed[, dims[1]])
		ymin <- min(embed[, dims[2]])
		ymax <- max(embed[, dims[2]])
		
		if (withumap){
			umap1 <- ggplot(data, aes_string(x=dims[1], y=dims[2], colour=names(data)[4])) +
				geom_point(shape=16, size=rel(0.4)) +
				coord_fixed(xlim=c(xmin, xmax), ylim=c(ymin, ymax)) +
				theme_bw() + 
				theme(panel.grid=element_blank())
			
			umap2 <- ggplot(data, aes_string(x=dims[1], y=dims[2], colour=names(data)[4])) +
				facet_wrap(names(data)[4], scales='fixed', drop=FALSE) +
				geom_point(shape=16, size=rel(0.4)) +
				coord_fixed(xlim=c(xmin, xmax), ylim=c(ymin, ymax)) +
				theme_bw() + 
				theme(panel.grid=element_blank(),
			...
			print( (umap1 | plot_spacer()) / umap2 )
		}

		all <- ggplot(data, aes_string(x=dims[1], y=dims[2])) +
			geom_density2d_filled(bins=binnum) +
			geom_point(shape=16, size=rel(0.3), alpha=0.15, colour='yellow') +
			coord_fixed(xlim=c(xmin, xmax), ylim=c(ymin, ymax)) +
			scale_fill_manual(values=colorRampPalette(c('black', '#DDAAFF'))(binnum)) +
		...
		if (relative || any(data[, nrow(.SD), by=data[[4]]]$V1 <= 1)) {
			split <- Reduce(`+`, lapply(levels(data[[4]]), function(s){
								if (nrow(data[data[[4]]==s, ]) >1 ) {
									ggplot(data[data[[4]]==s, ], aes_string(x=dims[1], y=dims[2])) +
										geom_density2d_filled(bins=binnum) +
										geom_point(shape=16, size=rel(0.3), alpha=0.15, colour='yellow') +
										scale_fill_manual(values=colorRampPalette(c('black', '#DDAAFF'))(binnum)) +
										coord_fixed(xlim=c(xmin, xmax), ylim=c(ymin, ymax)) +
										labs(fill="cell density", subtitle=s) +
										theme_minimal() +
										theme(panel.grid=element_blank(), 
													panel.background=element_rect(fill='black'),
													legend.position='none')
									...
									ggplot(data[data[[4]]==s, ], aes_string(x=dims[1], y=dims[2])) +
										# geom_density2d_filled(bins=binnum) +
										geom_point(shape=16, size=rel(0.3), colour='yellow') +
										# scale_fill_manual(values=colorRampPalette(c('black', '#DDAAFF'))(binnum)) +
										coord_fixed(xlim=c(xmin, xmax), ylim=c(ymin, ymax)) +
										labs(fill="cell density", subtitle=s) +
										theme_minimal() +
			...
			split <- ggplot(data, aes_string(x=dims[1], y=dims[2])) +
				facet_wrap(names(data)[4], scales='fixed', drop=FALSE) +
				geom_density2d_filled(bins=binnum) +
				geom_point(shape=16, size=rel(0.3), alpha=0.15, colour='yellow') +
				scale_fill_manual(values=colorRampPalette(c('black', '#DDAAFF'))(binnum)) +
		...
		print( (all | plot_spacer()) / split )
		
	} else {
		
		print("Warning: Insufficient number of cells for a 2D density plot calculation.")
	}
}


do_clusters <- function(so, d=1:50, resolution=0.8, spread=1, nneighbours=30, mindist=0.3, clusterplotonly=FALSE){
	...
	so <- RunUMAP(so, dims=d, n.neighbors=nneighbours, min.dist=mindist, spread=spread)

	so <- FindNeighbors(so, dims=d)
	so <- FindClusters(so, resolution=resolution, group.singletons=FALSE)
	...
	# 	print( p1 / p2 + plot_layout(guides='collect') )
		...
		plot_cell_density(so, cells = row.names(so@meta.data),
											split.by = 'Source', relative=FALSE, withumap=TRUE)
		plot_cell_density(so, cells = row.names(so@meta.data),
											split.by = 'Source', relative=TRUE, withumap=FALSE)
		...
		plot_cell_density(so, cells = row.names(so@meta.data),
											split.by = 'Phase', relative=TRUE, withumap=TRUE)
	}
	...
	p1 <- DimPlot(so, reduction='umap', label=TRUE, group.by='seurat_clusters') +
		...
		theme(panel.grid=element_blank(),
					legend.position='none')
	if (clusterplotonly) {
		p1 <- p1 + labs(subtitle=paste("n", nneighbours, "d", mindist, "s", spread, "r", resolution))
	}
...
do_de <- function(so, DEtabled=FALSE, lfcthresh=0.25, pctexpthresh=0.25, diffpct=-Inf, odr=outdir, pjn=projectname, suffix=NULL) {
	if (!DEtabled) {
		...
		suppressWarnings(so.markers <- fread(file=file.path(odr, paste0(pjn, '_clustermarkers_', suffix, '.tsv'))))
		so.markers[, V1 := NULL]   # data.frame rownames, already duplicated as the gene column
	}
	
	so.markers[, abslfc := abs(avg_logFC)]
	so.markers <- so.markers[! gene %in% c(mtgenes, ribogenes, cccore, ccextra),]
	setorder(so.markers, -avg_logFC)
	cm <- so.markers[, head(.SD, 18), by=cluster] # negative LFCs tend to not be cluster-specific, because of averaging across all the other clusters
	cm[, cluster := factor(as.character(cluster), levels=as.character(unique(so.markers$cluster[order(so.markers$cluster)])))]
	
	so@meta.data["cell"] <- rownames(so@meta.data) ## TODO remove after recompiling all checkpoints
	
	for (cl in levels(cm$cluster)){
		# cl <- levels(cm$cluster)[1]
		...
		plot_cell_density(so, cells=so@meta.data[so@meta.data$seurat_clusters == cl, "cell"],
														 split.by='Source', relative=FALSE, withumap=TRUE)
		...
		for (x in rollslice(subg, 9)) {
			...
			# Each marker split by source
			for (z in subg[x]){
				# z <- subg[x][1]
				print( (FeaturePlot(so, reduction='umap', features = z, coord.fixed=TRUE) | plot_spacer()) /
				       FeaturePlot(so, reduction='umap', features = z, coord.fixed=TRUE, split.by='Source') )
				
				plot_cell_density(so, cells=colnames(so@assays$RNA@data)[so@assays$RNA@data[z, ] > 0], 
													split.by='Source', withumap=TRUE, relative=TRUE)
			}
		
			## In larger blocks, without splitting by source
			...
			print( RidgePlot(so, features=subg[x], group.by='Source', stack=FALSE, same.y.lims=TRUE) & labs(y=NULL, x=NULL) )
...
if (!filtered){
	...
	minfeat <- rep(1000, length(sources))           # USER-INPUT
	names(minfeat) <- levels(sources)
	maxfeat <- rep(Inf, length(sources))            # USER-INPUT
	names(maxfeat) <- levels(sources)
	maxmols <- rep(30000, length(sources))          # USER-INPUT
	...
	maxmt <- rep(10, length(sources))                     # USER-INPUT
	names(maxmt) <- levels(sources)
	maxribo <- rep(50, length(sources))                   # USER-INPUT
...
if (!scanvar){ # varscan prints this already
	varscanplot(v, candidates)
}
```
...
# USER-INPUT        # c(minover1, kover1, minsum80, ksumto80, ksum2, kelbow, maxelbow, mindiff2, kdiff2)
# nvariables <- candidates['maxelbow'] 
nvariables <- 1000
...
topvariable <- clean_candidates(topvariable, verbose=TRUE,
																# USER-INPUT  # USER-INPUT    # USER-INPUT   # USER-INPUT     # USER-INPUT
																mito=mtgenes, ribo=ribogenes, cccore=cccore, ccextra=ccextra, expthresh=0.002)
...
The 1-D and 2-D density plots show how the principal components associate with treatments/sources, with the cell cycle, and with overall expression/detection metrics. Association with the treatments is desireable, association with the detection metrics is a confounding factor. Association with the cell cycle may be a confounding factor if not modelled out in the earlier section, unless the treatments affect the cycle.
...
## Use the density distributions to see which components encode for the treatment groups, cell cycle phases, and zero counts
pcs <- as.data.table(so@reductions$pca@cell.embeddings)
pcs[, Source := so@meta.data$Source]
pcs[, Phase := so@meta.data$Phase]
pcs[, NonZeros := so@meta.data$nFeature_RNA]
pcs[, Counts := so@meta.data$nCount_RNA]
pcs <- melt(pcs, variable.name="PC", value.name="Embedings", id.vars=c('Source', 'Phase', 'NonZeros', 'Counts'))

p1 <- ggplot(pcs, aes(x=Embedings, colour=Source)) +
	facet_wrap(~ PC, scales="free_x") +
	geom_density() +
	labs(title="PCs encoding the source") +
	theme_minimal()

print( p1 )

binnum=5
# p1 <- ggplot(pcs, aes(x=Embedings, y=NonZeros)) +
# 	facet_wrap(~ PC, scales="free_x") +
# 	# geom_point(shape=16, size=0.7, alpha=0.15) +
# 	geom_density_2d(bins=binnum) +
# 	# geom_density_2d_filled(bins=binnum) +
# 	scale_fill_manual(values=colorRampPalette(c('black', 'yellow'))(binnum)) +
# 	labs(title="PCs encoding the number of detected features") +
# 	theme_minimal() +
# 	theme(axis.text.x=element_text(angle=60, vjust=0.5, hjust=1))
# 
# print( p1 )
							
p1 <- ggplot(pcs, aes(x=Embedings, y=Counts)) +
	facet_wrap(~ PC, scales="free_x") +
	# geom_point(shape=16, size=0.7, alpha=0.15) +
	geom_density_2d(bins=binnum) +
	# geom_density_2d_filled(bins=binnum) +
	scale_fill_manual(values=colorRampPalette(c('black', 'yellow'))(binnum)) +
	labs(title="PCs encoding the volume of data") +
	theme_minimal() +
	theme(axis.text.x=element_text(angle=60, vjust=0.5, hjust=1))
...
p1 <- ggplot(pcs, aes(x=Embedings, colour=Phase)) +
	facet_wrap(~ PC, scales="free_x") +
	geom_density() +
	labs(title="PCs (still) encoding the cell cycle") +
	theme_minimal()

print( p1 )
```

Below are the airwise plots of the first few PCs.
The diagonal shows the distribution of observations along the PC, coloured by treatment. Below the diagonal are PCA plots coloured by treatment. Above the diagonal are PCA plots coloured by cell phase.
...
## Selected components
custom_pcs <- c(1:6, 8:12, 15:17, 20:25, 28, 30)    # USER-INPUT
...
if (scanumap){
	pdf(file.path(outdir, paste0(projectname, '_alldim_scan.pdf')))
	for (mindist in c(0.01, 0.3)) {
		for (spr in c(0.25, 1, 4)) {
			for (nneighbours in c(10, 30, 50)) {
				for (resolution in seq(0.4, 1.2, 0.2)) {
					do_clusters(so, d=d, resolution=resolution, spread=spr, mindist=mindist, nneighbours=nneighbours, clusterplotonly=TRUE)
				}
			}
		}
	}
	dev.off()
	# pdf(file.path(outdir, paste0(projectname, '_alldim_scanchoices.pdf')))
	# do_clusters(so, d=d, resolution=0.8, spread=4, mindist=0.3, nneighbours=50, clusterplotonly=TRUE)
	# do_clusters(so, d=d, resolution=0.8, spread=4, mindist=0.3, nneighbours=30, clusterplotonly=TRUE)
	# do_clusters(so, d=d, resolution=0.8, spread=4, mindist=0.3, nneighbours=10, clusterplotonly=TRUE)
	# do_clusters(so, d=d, resolution=0.8, spread=4, mindist=0.01, nneighbours=30, clusterplotonly=TRUE)
	# dev.off()
} else {
	nneighbours <- 30                                     # USER INPUT
	mindist <- 0.01                                        # USER INPUT
	resolution <- 0.8                                     # USER INPUT
	spr <- 4
	so <- do_clusters(so, d=d, resolution=resolution, spread=spr, mindist=mindist, nneighbours=nneighbours)
...
Aiming for `r varcutoff`% variance explained.

```{r umap2, fig.height=12, fig.width=12}
d <- dims[dims %in% 1:npc]

if (scanumap){
	pdf(file.path(outdir, paste0(projectname, '_topdim_scan.pdf')))
	for (mindist in c(0.01, 0.3)) {
		for (spr in c(0.25, 1, 4)) {
			for (nneighbours in c(10, 30, 50)) {
				for (resolution in seq(0.4, 1.2, 0.2)) {
	...
	dev.off()
	# pdf(file.path(outdir, paste0(projectname, '_topdim_scanchoices.pdf')))
	# do_clusters(so, d=d, resolution=0.6, spread=4, mindist=0.01, nneighbours=30, clusterplotonly=TRUE)
	# do_clusters(so, d=d, resolution=0.8, spread=4, mindist=0.01, nneighbours=50, clusterplotonly=TRUE)
	# do_clusters(so, d=d, resolution=0.6, spread=4, mindist=0.3, nneighbours=10, clusterplotonly=TRUE)
	# do_clusters(so, d=d, resolution=0.8, spread=4, mindist=0.3, nneighbours=50, clusterplotonly=TRUE)
	# dev.off()
} else {
	nneighbours <- 50                                     # USER INPUT
	mindist <- 0.01                                        # USER INPUT
	resolution <- 0.8                                     # USER INPUT
	spr <- 4
	so <- do_clusters(so, d=d, resolution=resolution, spread=spr, mindist=mindist, nneighbours=nneighbours)
...
if (scanumap){
	pdf(file.path(outdir, paste0(projectname, '_customdim_scan.pdf')))
	for (mindist in c(0.01, 0.3)) {
		for (spr in c(0.25, 1, 4)) {
			for (nneighbours in c(10, 30, 50)) {
				for (resolution in seq(0.4, 1.2, 0.2)) {
	...
	dev.off()
	# pdf(file.path(outdir, paste0(projectname, '_customdim_scanchoices.pdf')))
	# do_clusters(so, d=d, resolution=0.8, spread=4, mindist=0.01, nneighbours=30, clusterplotonly=TRUE)
	# do_clusters(so, d=d[-1], resolution=0.8, spread=4, mindist=0.01, nneighbours=30, clusterplotonly=TRUE)
	# do_clusters(so, d=d, resolution=0.8, spread=4, mindist=0.3, nneighbours=30, clusterplotonly=TRUE)
	# do_clusters(so, d=d[-1], resolution=0.8, spread=4, mindist=0.01, nneighbours=50, clusterplotonly=TRUE)
	# do_clusters(so, d=d, resolution=0.8, spread=4, mindist=0.3, nneighbours=50, clusterplotonly=TRUE)
	# do_clusters(so, d=d[-1], resolution=0.8, spread=4, mindist=0.3, nneighbours=50, clusterplotonly=TRUE)
	# do_clusters(so, d=d, resolution=0.8, spread=4, mindist=0.01, nneighbours=50, clusterplotonly=TRUE)
	# do_clusters(so, d=d[-1], resolution=0.8, spread=4, mindist=0.3, nneighbours=30, clusterplotonly=TRUE)
	# dev.off()
} else {
	nneighbours <- 50                                     # USER INPUT
	mindist <- 0.01                                        # USER INPUT
...
Genes up-regulated in each cluster, relative to the other clusters collectively.
...
Manually specified markers of interest.
...
for (y in names(priors)) {
	...
	for (x in rollslice(subg, 9)) {
		...
		for (z in subg[x]){
			...
			plot_cell_density(so, cells=colnames(so@assays$RNA@data)[so@assays$RNA@data[z, ] > 0], 
												split.by='Source', withumap=FALSE, relative=TRUE)
		}
...
Aggregated scores of expression for specified gene groups.

```{r sig, fig.height=12, fig.width=12}
signatures <- lapply(as.list(signatures), function(x){ x[!is.na(x)] })
so <- AddModuleScore(so, features=signatures, search=TRUE, assay='RNA', name='GeneSignature')
names(so@meta.data)[(length(so@meta.data) - length(signatures) + 1):length(so@meta.data)] <- paste0('Signature_', names(signatures))


l <- lapply (names(so@meta.data)[(length(so@meta.data) - length(signatures) + 1):length(so@meta.data)], function(x) {
	p1 <- FeaturePlot(so, features=x, reduction="umap", cols=c('yellow', 'blue'), coord.fixed=TRUE)
	p2 <- FeaturePlot(so, features=x, reduction="umap", cols=c('yellow', 'blue'), coord.fixed=TRUE, split.by="Source")
	(p1 | plot_spacer()) / p2
})
print(l)

# pdf(file.path(outdir, paste0(projectname, '_signatures.pdf')))
# print(l)
# dev.off()
```
