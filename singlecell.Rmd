...
projectname = 'OT1_Kdm2b-Rc3h1_ultimate'
outdir <- '~/Desktop/markus_SC'

# Markers of interest
# USER-INPUT
positivefilter <- c('Cd8a', 'Cd8b1', 'Cd3d')     # CD8 Tcell markers
negativefilter <- c('Cd4')
# KOfilter <- c(control=NA_character_, Kdm2b='Kdm2b', Rc3h1='Rc3h1')    # names must match the condition names in datadirs
KOfilter <- c(control=NA_character_, Kdm2b=NA_character_, Rc3h1=NA_character_)    # names must match the condition names in datadirs
filtercheck <- c('Cd2', 'Cd3e', 'Kdm2b', 'Rc3h1')
priors <- list(selection=c(positivefilter, negativefilter, filtercheck, KOfilter),
...
# USER_INPUT
cccore <- read.table('~/Desktop/markus_SC/cellcycle.txt', stringsAsFactors=FALSE)[[1]]

# Core cell cycle genes by phase.  According to Seurat vignette.          
# USER_INPUT
...
plot_variancemeans <- function(so, features=NULL, title=NULL){
	hvf.info <- HVFInfo(object = so, assay = NULL, selection.method = NULL,
        status = TRUE)
	if (!is.null(features)){
		hvf.info["var.status"] <- rownames(hvf.info) %in% features
		top <- rownames(hvf.info[VariableFeatures(so),])
		top <- head(top[top %in% features], 25)
		clr <- 'red'
	} else {
		...
		clr <- 'dodgerblue'
		title='Selection of highest variance'
	}
	hvf.info <- hvf.info[order(hvf.info$variance.standardized, decreasing=TRUE),]
	limitrank <- which(rownames(hvf.info) == head(rownames(hvf.info[top,]), 1)) # Highest rank of the selection
	...
	p1 <- ggplot(hvf.info,
		...
		geom_text_repel(data=hvf.info[top,], aes(label=row.names(hvf.info[top,])),
										min.segment.length=0, size=rel(2.5), force=10, colour=clr, max.overlaps=1e4) +
		scale_x_log10() +
		annotation_logticks(base=10, sides='b') +
		scale_colour_manual(values=c('grey50', 'red'), name='Selected') +   # TRUE/"yes" are ordered second after FALSE/'no'
		labs(x='Average Expression', y='Standardized Variance', title=title) +
		theme_classic()
...
clean_candidates <- function(x, mito=NULL, ribo=NULL, cccore=NULL, ccextra=NULL, verbose=TRUE){
	# Remove cell cycle genes from the pool.
	if (!is.null(ribo)){
		if (verbose) message('Removing ribosomal genes from the variables.')
		x <- x[! x %in% ribo]
	...
	if (!is.null(mito)){
		if (verbose) message('Removing mitochondrial genes from the variables.')
		x <- x[! x %in%mito]
		if (verbose) message(paste(length(x), 'features remain.'))
	} else if (verbose){
		sel <- x[x %in% mito]
	...
	if (!is.null(cccore)){
		if (verbose) message("Removing any cell cycle genes from the variables.")
		x <- x[! x %in% cccore]
		if (verbose) message(paste(length(x), 'features remain.'))
	} else if (verbose){
		sel <- x[x %in% cccore]
	...
	if (!is.null(ccextra)){
		if (verbose) message("Removing genes closely correlated with the cell cycle from the analysis pool:")
...
varscan <- function(so, n, exclude=NULL, varcutoff=varcutoff) {
	message(paste("Cumulative variance explainec cutoff:", varcutoff))
	...
	p2 <- ggplot(v) +
		...
		geom_vline(xintercept=candidates['minsum80'], linetype='dashed', colour='magenta', alpha=0.2) +
		geom_vline(xintercept=candidates['ksum80'], linetype='dotted', colour='magenta') +
		geom_line(aes(x=n, y=sum_to_80, colour='cumulatively reaching 80% variance explained')) +
		...
		scale_x_sqrt(breaks=c(100,500,1000,2000,5000,10000,15000,20000)) +
		scale_colour_manual(values=c(`cumulatively reaching variance explained cutoff`='magenta', 
...
do_clusters <- function(so, d=1:50, resolution=0.8, nneighbours=30, mindist=0.3){
  ...
  p2 <- DimPlot(so, reduction='umap', group.by='Source', split.by='Source') +
	...
	p2 <- ggplot(melt(as.data.table(frac1), id.vars='Cluster', value.name='pct', variable.name='Source'),
		...
		geom_tile() +
		geom_text(aes(label=round(pct, 1)), size=rel(2.5)) +
...
# if (!filtered) {
	...
	for (s in unique(so@meta.data$Source)){
		message(s)
		print( summary(so@meta.data[so@meta.data$Source==s, c('nFeature_RNA', 'nCount_RNA', 'percent.mt', 'Source')]) )
	}
...
```{r qc_inpu2, fig.width=10, fig.height=6}
# if (!filtered) {
...
p1 <- plot_variancemeans(so, mtgenes, 'Mitochondrial variances')
p2 <- plot_variancemeans(so, ribogenes, 'Ribosomal variances')
print(p1 / p2 + plot_layout(guides='collect'))

if (!filtered) {
	# Remove unwanted cells
	minfeat <- 1000      # USER-INPUT
	maxfeat <- 5000      # USER-INPUT
	maxmt <- 8           # USER-INPUT
	maxribo <- 50        # USER-INPUT
	maxmols <- 30000     # USER-INPUT
	minmols <- minfeat   # USER-INPUT    (it can be bigger than minfeat, but it doesn't make sense to be smaller)
	
	message('Removing cells with high abundance of ribosomal and mitochondrial genes.')
	message(paste('Selected lower threshold:', minfeat, 'features'))
	message(paste('Selected upper cutoff:', maxfeat, 'features'))
	message(paste('Selected lower threshold:', minmols, 'molecules'))
	message(paste('Selected upper cutoff:', maxmols, 'molecules'))
	message(paste('Max mitochondrial contamination allowed:', maxmt, '%'))
	message(paste('Max ribosomal presence allowed:', maxribo, '%'))
	
	# Remove cells with high/low number of features
	so <- subset(so, subset = nFeature_RNA > minfeat & nFeature_RNA < maxfeat &
  # and cells with too many sequenced molecules
  	            nCount_RNA < maxmols & nCount_RNA >= minmols &
	# and cells with high mitochondrial/ribosomal presence
							 	percent.mt < maxmt & percent.ribo < maxribo)
...
if (custom_filter) {
	if(!filtered) {
		expthresh <- 2.5      # USER-INPUT min log expression level
		
		if (!is.null(positivefilter)){
			sel <- so@assays$RNA@data[positivefilter, ] >= expthresh
	
			if(is.null(dim(sel))){
				so <- subset(so, cells = colnames(so)[sel] )
			} else {
				## USER_INPUT choose filtering method
				
				# message(paste('Keep cells expressing ALL of', paste(positivefilter,  collapse=','), 'above', expthresh))
				# so <- subset(so, cells = colnames(so)[colSums(sel) == length(positivefilter)] )
				
				message(paste('Keep cells expressing ANY of', paste(positivefilter,  collapse=','), 'above', expthresh))
				so <- subset(so, cells = colnames(so)[colSums(sel) >= 1] )
			}
		}
		
		if (!is.null(negativefilter)) {
			# USER-INPUT choose filtering method
			
			# message(paste('Reject cells expressing', paste(negativefilter,  collapse=','), 'above', expthresh))
			# sel <- so@assays$RNA@data[negativefilter, ] < expthresh
			
			message(paste('Reject cells expressing', paste(negativefilter,  collapse=',')))
			sel <- so@assays$RNA@data[negativefilter, ] == 0
...
Individual for each input.

```{r ko}
if (custom_filter){
	if (!filtered && any(!is.na(KOfilter))) {
		print( VlnPlot(so, features=KOfilter, split.by='Source') & labs(x='Samples') )
		
		expthresh <- 2.5      # USER-INPUT min log expression level
		message(paste("Retaining detection <=", expthresh))
		
		cellsbysource <- lapply(names(KOfilter), function(x){
			colnames(so)[so@meta.data$Source == x]
		})
		names(cellsbysource) <- names(KOfilter)
		
		sel <- lapply(names(KOfilter), function(x){
			if (!is.na(KOfilter[x])) {
				cellsbysource[[x]] [ which(so@assays$RNA@data[KOfilter[x], cellsbysource[[x]]] <= expthresh) ]
			} else {
				cellsbysource[[x]]
			}
		})
		names(sel) <- names(KOfilter)
		
		v <- paste0(round(vapply(sel, length, integer(1)) / vapply(cellsbysource, length, integer(1)), 3) * 100, "%")
		names(v) <- names(KOfilter)
		print( v )
		
		so <- subset(so, cells=unlist(sel))
		print( VlnPlot(so, features=KOfilter, split.by='Source') & labs(x='Samples') )
...
if (!is.null(s.genes) && !is.null(g2m.genes)) {
	...
	message("No cell cycle gene lists were provided.")
}
```
...
if (!filtered){
	...
	cm <- sparse.cor(t(so@assays$RNA@data))  # This takes several minutes (regular matrix takes forever)...
	cm <- as.matrix(cm)
	cm <- cm[cccore,]
	cml <- cm >= corthresh
	sel <- apply(cml, 2, function(x){ any(x, na.rm=TRUE) })
	message(paste(sum(sel), "genes correlate with coeficient >", corthresh, "with the", length(cccore), "provided cell cycle genes:"))
	...
	ccextra <- readRDS( file = file.path(outdir, paste0(projectname, '_ccextra.RDS')))
}

p1 <- plot_variancemeans(so, features=cccore, title='Core cell cycle genes')
p2 <- plot_variancemeans(so, features=ccextra, title='Cell cycle correlated genes')
...
```{r ccpost1, fig.height=12, fig.width=12}	
if (regress_cycle){
	so <- FindVariableFeatures(so, nfeatures=1000)
	sel <- VariableFeatures(so)
	so <- RunPCA(so, features = sel)
...
if (scanparam){
	v <- varscan(so, n, exclude=c(mtgenes, ribogenes, cccore, ccextra))
...
# Choose PCA/UMAP/clustering parameters to use.
nvariables <- candidates['mindiff2']  # USER-INPUT, data-dependent
dims <- 1:50                          # Obsolete as user-defined sub-selection, but baked into code that I don;t want to fix right now.
                                      #   Look at custom_pcs in the pca_plot chunk a bit further down instead.
...
# USER-INPUT   (choose what to exclude)
# topvariable <- clean_candidates(topvariable, mito=mtgenes, ribo=ribogenes, cccore=cccore, verbose=TRUE)
topvariable <- clean_candidates(topvariable, mito=mtgenes, ribo=ribogenes, cccore=cccore, ccextra=ccextra, verbose=TRUE)


top <- head(topvariable, 25)
hvf.info <- HVFInfo(object = so, assay = NULL, selection.method = NULL,
        status = TRUE)
hvf.info["var.status"] <- row.names(hvf.info) %in% topvariable

hvf.info["var.status_text"] <- NA_character_
hvf.info[hvf.info$var.status, "var.status_text"] <- paste0("yes (",  sum(hvf.info$var.status), ")")
hvf.info[!hvf.info$var.status, "var.status_text"] <- paste0("no (", sum(!hvf.info$var.status), ")")

hvf.info <- hvf.info[, c(1, 3, 6)]

p1 <- ggplot(hvf.info,
			 aes(x=mean, y=variance.standardized, colour=var.status_text, label=row.names(hvf.info))) +
	geom_point(shape=16) +
	geom_text_repel(data=hvf.info[top,], aes(label=row.names(hvf.info[top,])),
...
```{r pca_plot_pre, fig.height=12, fig.width=12}
pc_vars <- Stdev(object = so, reduction = 'pca') ^ 2  # From stdevs to % of total variance.
pc_vars <- 100 * pc_vars / sum(pc_vars)
...
## Selected components
custom_pcs <- c(3:6, 8:13, 17, 19, 23:25)                         # USER-INPUT
message(paste("Manually chosen PCs for UMAP projection:", paste(custom_pcs, collapse=", "), ", accounting for", round(sum(pc_vars[custom_pcs]), 2), "% of the variation."))

# plot_grid_pcs(so, comps= custom_pcs[c(1:5)])
plot_grid_pcs(so, comps= c(6, 8, 10, 11))                        # USER-INPUT
```

## UMAP

Tuned to break up clusters that consist of multiple non-overlapping subgroups that are differentially present between conditions,
potentially at the cost of also breaking up clusters that are more homogeneous.

### Using all 50 components

```{r umap0, fig.height=12, fig.width=12}
d <- dims[dims %in% 1:length(pc_vars)]
resolution <- 1.8                                     # USER INPUT
...
resolution <- 1.1                                     # USER INPUT
nneighbours <- 30                                     # USER INPUT
mindist <- 0.1                                       # USER INPUT

so2 <- do_clusters(so, d=d, resolution=resolution, mindist=mindist, nneighbours=nneighbours)

# Checkpoint
saveRDS(so2, file = file.path(outdir, paste0(projectname, '_topdim.RDS')))
```
...
resolution <- 1.2                                     # USER INPUT
nneighbours <- 30                                     # USER INPUT
mindist <- 0.1                                       # USER INPUT

so3 <- do_clusters(so, d=d, resolution=resolution, mindist=mindist, nneighbours=nneighbours)

# Checkpoint
saveRDS(so3, file = file.path(outdir, paste0(projectname, '_customdim.RDS')))
...
<!-- do_de(so, DEtabled=DEtabled, lfcthresh=lfcthresh, pctexpthresh=pctexpthresh, diffpct=diffpct, suffix='alldim') -->
<!-- ``` -->

<!-- ### Top informative components -->

<!-- ```{r clustmark2, fig.height=10, fig.width=10} -->
<!-- do_de(so2, DEtabled=DEtabled, lfcthresh=lfcthresh, pctexpthresh=pctexpthresh, diffpct=diffpct, suffix='topdim') -->
<!-- ``` -->

### Selected components

```{r clustmark3, fig.height=10, fig.width=10}
do_de(so3, DEtabled=DEtabled, lfcthresh=lfcthresh, pctexpthresh=pctexpthresh, diffpct=diffpct, suffix='customdim')
```
...
for (y in names(priors)) {
	...
	print( p1 <- plot_variancemeans(so, features=subg, title=NULL) )
