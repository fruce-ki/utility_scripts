...
DT["spike_factor"] <- target / DT$spike_counts  # Multiplication factor to apply so as to get all spikes to the same value
# DT["subject_scaled"] <- DT$subject_counts * DT$spike_factor

write.table(DT[, c("row_ID", "spike_factor")], file=sub('txt|csv|tsv', 'factors.txt', args[1], perl=TRUE), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE)
