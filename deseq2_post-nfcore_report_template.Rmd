...
params:
  ...
  genesets: ['/SCRATCH/REFERENCES/msigdb_v2023.2.Hs_GMTs/h.all.v2023.2.Hs.symbols.gmt', '/SCRATCH/REFERENCES/msigdb_v2023.2.Hs_GMTs/c5.all.v2023.2.Hs.symbols.gmt', '/SCRATCH/REFERENCES/msigdb_v2023.2.Hs_GMTs/c2.all.v2023.2.Hs.symbols.reduced.gmt', '/SCRATCH/REFERENCES/msigdb_v2023.2.Hs_GMTs/c1.all.v2023.2.Hs.symbols.gmt']
  ...
  min_lfc: 0.5          # minimum log2 fold-change  for DE to be accepted
  max_q: 0.05           # FDR level for DE and GSEA
  genelist : ''         # optional file with custom gene list to highlight instead of DE
  pad: true             # pad zero counts for plots (log-scale axes hide the non-expressed genes)
---
...
fwrite(deseq, file = file.path(params$da_path, 'tables', 'differential', paste0(params$comparison, '.deseq2.results.csv')), sep=',', col.names = TRUE) # csv instead of tsv. I don't want to overwrite the original. It would mess up re-runs.
```
...
Gene sets are considered enriched if they meet the criterion: FDR < `r params$max_q`. When multiple gene-set resources are queried, the default multiple-testing correction is inadequate, as it is performed for each resource separately. Therefore a new correction is carried out to encompass the real total number of tests. The genes are ranked by the shrunken log2 fold-change.

Only gene-sets that meet the FDR cutoff are shown in the plots and tables below. Out of those, only the highest nerchment scores are labeled in the plots, for legibility.
...
run_fgsea <- function(gmts, x, tail, q, comparison, title) {
  gsea <- Reduce(rbind, 
                 lapply(gmts, 
                        function(gmt) {
                          ...
                          fwrite(res, file = file.path(params$da_path, 'tables', 'gsea', comparison, paste0(title, '.fgsea_', tail, '_', sub('.gmt', '', basename(gmt)), '.tsv')),
  ...
  gsea[, padj := p.adjust(pval, 'BH')] # If only one source, this should match padj. But with this redundancy, I don't have to code it separately than having multiple sources..
}

plot_fgsea <- function(res, title, ncol, n) {
  res[, absNES := abs(NES)]
  setorder(res, -absNES)
  if (any(res$NES < 0)) {
    res[NES > 0, split := 'NES > 0']
    res[NES < 0, split := 'NES < 0']
    
    p <- ggplot(res, aes(x = NES, y = -log10(padj), label = pathway)) +
          facet_grid(resource ~ split, scales = 'free') +
          # geom_vline(xintercept = 0, linetype = 'dashed') + 
          geom_point(aes(size = size), shape=21, fill='gold', colour = 'darkorange', alpha=0.6) +
          geom_text_repel(data = res[, head(.SD, floor(n)), by = c('resource', 'side')], 
...
Table of all qualifying gene-sets, across all the gene-set libraries used:

```{r twotail_tb}
datatable(gsea2[, .(side, pathway, padj, NES, size, leadingEdge)])
```

### NES & Significance

```{r twotail_volcano, fig.height=40, fig.width=20}
p_rich2 <- plot_fgsea(gsea2, paste0(params$comparison, '.absLFC'), 1, 50)
...
# Re-do the multiple-testing correction to encompass the total number of tests.
gsea1[, padj := p.adjust(pval, method = 'BH')]
...
```{r onetail_tb}
datatable(gsea1[, .(side, pathway, padj, NES, size, leadingEdge)])
```

### NES & Significance

```{r onetail_volcano, fig.height=40, fig.width=20}
p_rich1 <- plot_fgsea(gsea1, params$comparison, 1, 30)
