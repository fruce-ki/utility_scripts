...
corels2p <- my_pairwise_internal_corels(vstmat, colnames(tpm), method="spearman",
                                       minMean=0, minSingle=0)
...
do_de_plots <- function(contrast, dds = deobj, rpms = tpm, regcnt=vstmat, ntop = params$ntop, addPrefix = params$longlabel,
  ...
  lfc <- results(dds, test="Wald", contrast=contrast, independentFiltering = filterBM, alpha=pcutoff, lfcThreshold=ifelse(is.null(params$fc4p) || !params$fc4p, 0, params$lfcthreshold))
  ...
  ph1 <- ggplot(selcnt, aes(x=sample, y=name, fill=log10(selcnt[[PMtype]]))) +
            geom_tile() +
            # facet_grid(. ~ type) +
            scale_fill_gradient(high="yellow", low="grey10", na.value = "black", name=paste0("log10(",PMtype,")")) +
            coord_equal(ratio=1) +
            labs(title=ctrname) +
  ...
  ph2 <- ggplot(selvst, aes_string(x="sample", y="name", fill=ifelse(rlog, "rlog_count", "VST_count"))) +
            geom_tile() +
            # facet_grid(. ~ type) +
            scale_fill_gradient(high="yellow", low="grey10", na.value = "black") +
            coord_equal(ratio=1) +
            labs(title=ctrname) +
            theme(axis.text.x = element_text(angle=90, hjust=1, vjust=0.5),
                  panel.grid = element_blank())
  ...
  pe4 <- ggplot(NULL, aes(x=refMeanTPM + zeros, y=log2FoldChange, label=name, colour=category)) +
      geom_hline(yintercept = lfcthreshold, linetype="dashed",  colour="steelblue1") + 
      geom_hline(yintercept = -lfcthreshold, linetype="dashed",  colour="steelblue1") + 
      ...
      labs(title=ctrname, x=paste('reference Mean', PMtype), y='log2( FC )') +
      theme(aspect.ratio=1, legend.position='none',  panel.grid.minor = element_blank())
  
  pe4b <- ggplot(NULL, aes(x=refMean + zerosb, y=log2FoldChange, label=name, colour=category)) +
      geom_hline(yintercept = lfcthreshold, linetype="dashed",  colour="steelblue1") + 
      geom_hline(yintercept = -lfcthreshold, linetype="dashed",  colour="steelblue1") + 
      geom_point(data=outtab[category == highlight[1], ], alpha=0.4, size=rel(0.8),  colour="black") +
      geom_point(data=outtab[category == highlight[2], ], alpha=0.7, size=rel(0.8),  colour="black") +
      ...
      scale_colour_manual(values=hlcol) +
      annotation_logticks(base=10, sides='lr') +
      labs(title=ctrname, x='reference Mean Reads', y='log2( FC )') +
      theme(aspect.ratio=1, legend.position='none',  panel.grid.minor = element_blank())
  ...
  if (addPrefix) {
    ...
    setnames(custom, sub('TPM|RPM', PMtype, names(custom)))
...
```{r expov, warning=FALSE, fig.height=13.5, fig.width=15}
for (res in resall) {
  # res <- resall[[1]]
...
if (!is.null(params$comparisons)) {
  ...
  pallexp <- ggplot(allhitdf, aes(y=Feature, x=Sample, fill=log10(RPM))) +
    geom_tile() +
    scale_fill_gradient(high="yellow", low="grey10", na.value='black', paste('log10', colsuffix)) +
    coord_equal(ratio=1) +
    labs(y='', x='') +
    theme(axis.text.x = element_text(angle=90, hjust = 1, vjust = 0.5, size=rel(1)),
          axis.text.y = element_text(size=rel(0.6)),
