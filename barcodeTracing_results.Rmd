...
```{r pdfstart}
pdf('/Volumes/groups/obenauf/Kimon_Froussios/chris/YUMM17_barcode_tracing/paper/experiment1.pdf', paper="a4")
```
...
counts[, bc_id := order(Reference, decreasing=TRUE)]
setkey(counts, bc_id)
...
sampleNames <- c("Reference", "UT_313", "UT_319", "UT_321", "UT_322", "D+T_312", "D+T_314", "D+T_316", "D+T_317", "D+T_320", "D+T_323")
samples_tidy <- data.frame(name=factor(sampleNames, levels=sampleNames, ordered=TRUE), 
                           treatment=c('ref', rep('UT', 4), rep('DT', 6)))
```
...
ggplotly(
  ggplot(data=tmp, aes(x=Sample, y=Proportion, fill=sharedFill, group="A", label=bc_id)) +
