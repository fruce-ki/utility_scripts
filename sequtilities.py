...
# import Levenshtein as lev   # import within the functions to bypass temporarily missing modules
import pysam
...
def gtf2pandas(flist):
	...
	for gtf in input:
		...
		#gtf.set_index("parent_id", inplace=True)
		result.append(gtf)
	return result
...
def gtf2premrna(gtfs, filter=True):
	...
	result = []
	for g in gtfs:
		# Aggregate. Each gene likely has multiple GTF rows, but we are creating exactly one new entry per gene.
		grped = g.groupby("parent_id")
		gnum = len(grped)
		pres = pd.DataFrame( 
			{"chr" : grped.head(1)["chr"].tolist(),
				"source" : ['minmaxCoords'] * gnum,
				"feature" : ['exon'] * gnum,  # for the benefit of existing 3rd-party parsers
				"start" : grped["start"].min(),
				"stop" : grped["stop"].max(),
				"score" : ['.'] * gnum,
				"strand" : grped.head(1)["strand"].tolist(),
				"phase" : ['.'] * gnum,
				"attributes" : ['transcript_id \"' + gid + '_pre\"; gene_id \"' + gid + '\";' for gid in grped.head(1)["parent_id"]] },
			index = grped.head(1)["parent_id"]
		)
		# Filter
		if (filter):
...
def demuxWAnchor(bam, barcodes, outputdir='./process/fastq', tally=None, anchorSeq='TTCCAGCATAGCTCTTAAAC', anchorRegex=False, smm=2, bcOffset=-4, bcmm=1, guideLen=20, abort=30, qualOffset=33, unmatched=False, trimQC=False):
	...
	for r in samin.fetch(until_eof=True):
...
def filter_bam_by_region(flist, regions, outfiles):
	...
	for i, (myfile, myalias) in flist.enum():
		...
		for record in samin.fetch(until_eof=True):
...
def unpairBam(flist, outfiles):
	"""Remove indicators if pairedness from BAM file entries.
	...
	for i, (myfile, myalias) in flist.enum():
		...
		for record in samin.fetch(until_eof=True):
			newflag = 0
			if record.is_paired:
				newflag += (4 if record.is_unmapped else 0)
				newflag += (16 if record.is_reverse else 0)
				newflag += (256 if record.is_secondary else 0)
				newflag += (512 if record.is_qcfail else 0)
				newflag += (1024 if record.is_duplicate else 0)
				newflag += (2048 if record.is_supplementary else 0)
				record.flag = newflag
			samout.write(record)
...
def main(args):
	...
	parser.add_argument('--samUnpair', action='store_true',
								help="Convert a paired-end BAM file to single-end, by stripping the relevant flags")
	...
	# UNPAIR a BAM file
	elif params.samUnpair:
		unpairBam(flist, outfiles)
		if params.STDERRcomments:
			sys.stderr.write(ml.donestring("un-pairing reads in BAM"))
	...
	# FILTER BAM by overlap to given REGIONS
