...
deseq[, absl2fc := abs(log2FoldChange)]
...
pbias <- 0.75    # bias point selection to prioritize pvalue over fold-change (< 1) or vice verse (> 1)
...
deseq[, radius := sqrt((mlog10p / ppen) ^ 2 + log2FoldChange ^ 2)]
deseq[, rank := rank(-radius, ties.method = 'average', na.last = TRUE)]
deseq[(fltr_de_pass), de_rank := rank(-radius, ties.method = 'average', na.last = TRUE)]
deseq[, absl2fc := NULL]
deseq[, mlog10p := NULL]
...
if (!is.null(params$genelist)) {
  ...
  markgenes <- deseq[(de_rank %in% 1:params$highlight_n), gene_id]
  if (length(markgenes) <= 1) {                                                                       # if there isn't much to show
    markgenes <- c(markgenes, deseq[(rank %in% 1:(params$highlight_n - length(markgenes))), gene_id]) # add some context of the nearest misses
  }
}
...
cat('Metadata of the relevant samples:\n')
...
`fltr_pass` represents genes that meet all of the criteria: FDR < `r params$max_q` , abs(L2FC) > `r params$min_lfc`, mean raw count in either condition > `r params$min_count`, and mean TPM in either condition > `r params$min_tpm`.

```{r volcano, fig.heigh=15, fig.width=15}
volcXY <- unique(DT[, .(gene_id, gene_name, padj, log2FoldChange, fltr_abund_pass, fltr_de_pass, fltr_pass, mark)])
...
```{r ma, fig.heigh=30, fig.width=10}
maXY <- unique(DT[mainvar == 'reference', .(gene_id, gene_name, count_type, condGeneMean, log2FoldChange, fltr_pass, mark)])
...
p_ma <- ggplot(maXY, aes(x = condGeneMean, y = log2FoldChange, colour = fltr_pass, label = gene_name)) +
  ...
  geom_hline(yintercept = 0, linetype = 'dash') +
  ...
  scale_x_log10() +
  scale_colour_manual(values = c('grey50', '#2244FF')) +
  labs(title = params$comparison, y = 'Reference abundance') +
  theme(legend.position = 'bottom')
...
abundXY <- dcast(unique(DT[count_type %in% c('raw', 'tpm', 'vst'), .(gene_id, gene_name, count_type, condGeneMean, fltr_abund_pass, fltr_de_pass, fltr_pass, mark, mainvar)]), '... ~ mainvar', value.var = 'condGeneMean')
...
The dotted lines represent the abundance thresholds that will be applied in accepting the DE results: mean raw count in either condition > `r params$min_count`, and mean TPM in either condition > `r params$min_tpm`.
...
```{r heatmap, fig.heigh=40, fig.width=15 }
heatXY <- DT[(mark), .(gene_name, sample, abundance, condGeneMean, count_type, mainvar, log2FoldChange)]
...
p_heat <- (ggplot(heatXY[count_type == 'tpm', ], aes(x = sample, y = gene_name, fill = log10(abundance))) +
          ...
          (ggplot(heatXY[count_type == 'normalised', ], aes(x = sample, y = gene_name, fill = log10(abundance))) +
            facet_wrap(~ count_type, scales = 'free') +
            geom_tile() +
            scale_fill_gradient(low = 'black', high = 'yellow') +
            theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) ) +
