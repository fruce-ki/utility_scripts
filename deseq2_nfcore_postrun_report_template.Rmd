...
params:
  ...
  min_count: 10         # minimum raw count
  min_tpm: 5            # minimum TPM
  min_lfc: 0.5          # minimum log2 fold-change 
  max_q: 0.05           # FDR level
  genelist : null                                                                   # file with custom gene list to highlight instead of DE
  pad: true             # pad zero counts for plots
---
...
rawpad <- min(DT[count_type == 'raw' & abundance > 0, abundance], na.rm = TRUE) / 10
DT[count_type == 'raw', abundance := abundance + rawpad]
message('Zero-padding the raw counts by ', rawpad)
tpmpad <- min(DT[count_type == 'tpm' & abundance > 0, abundance], na.rm = TRUE) / 10
                 ...
                 lapply(gmts, 
                        function(gmt) {
                          ...
                          res <- fgsea(pathways = pathways, stats = x, 
                                       minSize = 15, maxSize = 500, scoreType = tail, nPermSimple = 1000)
...
```{r twotail_plots, fig.height=40, fig.width=20}
...
Looking for gene-sets enriched in either the up-regulated or down-regulated genes.
...
```{r onetail_plots, fig.height=40, fig.width=20}
