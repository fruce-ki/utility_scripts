...
params:
  ...
  genesets: null                        # location of GMT files used for GSEA
...
```{r gsea, fig.height=6, fig.width=10}
...
rn <- ranks$gene_name
ranks <- ranks$absSLFC     # Up- and down-regulated genes from the same pathway shouldn't work against each other by being at opposite ends of the rank.
names(ranks) <- rn

gsea <- Reduce(rbind,
               lapply(c('/SCRATCH/REFERENCES/msigdb_v2023.2.Hs_GMTs/h.all.v2023.2.Hs.symbols.gmt',
                        '/SCRATCH/REFERENCES/msigdb_v2023.2.Hs_GMTs/c5.all.v2023.2.Hs.symbols.gmt',
                        '/SCRATCH/REFERENCES/msigdb_v2023.2.Hs_GMTs/c2.all.v2023.2.Hs.symbols.gmt'),
                      function(gmt) {
                        pathways <- gmtPathways(gmt)
                        fgseaRes <- fgsea(pathways, ranks, minSize = 15, maxSize = 500, scoreType = 'pos')
                        setorder(fgseaRes, padj, NES)
                        
                        fwrite(fgseaRes, file = file.path(params$da_path, 'tables', 'gsea', params$comparison, paste0('fgsea_', basename(gmt), '.tsv')),
