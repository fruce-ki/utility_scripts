...
my_pairwise_internal_corels <- function(mat, samples, method = "pearson", rds=NULL, txs=3, minMean=0, minSingle=0, loopVal="default") {
  ...
  if (!is.null(rds) && loopVal == "default") {
...
do_pca <- function(countsmat, covars, scale = TRUE, center = TRUE, rds=NULL, topgenes=NULL, exclude=NULL, minMean=0, minSingle=0, ntop=10) {
  # countsmat <- log10(tpm); covars <- covars
  # scale = TRUE; center = TRUE; rds=NULL; topgenes=NULL; exclude=NULL; minMean=0; minSingle=0; ntop=10
  ...
  pvar1 <- ggplot() +
      geom_point(data=genevar[!selected & excluded, ], aes(x=Mean, y=StDev/Mean), shape=16, size=0.8, alpha=0.5, colour='grey70') +
...
if (params$loopVal == "default")  # only output a master table, no point repeating subsets of it
...
corels1 <- my_pairwise_internal_corels(log10(tpm[topgenes,]), covars$Sample, method="pearson",
                                      minMean = 0, minSingle = 0, loopVal=params$loopVal,
                                      rds = file.path(params$RDSdir,
                                                      sub(".txt|.tsv", "_corrP.RDS", basename(params$cts))))
corels2 <- my_pairwise_internal_corels(log10(tpm[topgenes, ]), covars$Sample, method="spearman",
                                      minMean = 0, minSingle = 0, loopVal=params$loopVal,
                                      rds = file.path(params$RDSdir,
...
corels1b <- my_pairwise_internal_corels(log10(tpm[topgenes2, ]), covars$Sample, method="pearson",
                                      minMean = 0, minSingle = 0, loopVal=params$loopVal,
                                      rds = file.path(params$RDSdir,
                                                      sub(".txt|.tsv", "_corrPtop.RDS", basename(params$cts))))
corels2b <- my_pairwise_internal_corels(log10(tpm[topgenes2, ]), covars$Sample, method="spearman",
                                      minMean = 0, minSingle = 0, loopVal=params$loopVal,
                                      rds = file.path(params$RDSdir,
...
```{r pca}
all_pca <- do_pca(log10(tpm), covars, ntop = params$ntop,
...
```{r pcab}
all_pcab <- do_pca(log10(tpm), covars, ntop = params$ntop,
