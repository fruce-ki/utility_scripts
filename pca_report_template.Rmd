...
params:
  ...
  outdir: NULL                        # save path for intermediate data
...
do_pca <- function(countsmat, covars, scale = TRUE, center = TRUE, rds=NULL, topgenes=NULL, exclude=NULL, minMean=0, minSingle=0, ntop=10) {
  ...
  # scale = TRUE; center = TRUE; rds = file.path(params$outdir, sub(".txt|.tsv", "_corrP.RDS", basename(params$cts))); topgenes=NULL; exclude=NULL; minMean=0; minSingle=0; ntop=10
...
n <- nrow(params$tpm)
topgenes <- rownames(params$counts)[(rowMeans(params$counts) >= params$minMean) | (rowSums(params$counts >= params$minSingle) > 0)]
...
lowcnt <- rownames(params$counts)[(rowMeans(params$counts) < params$minMean) & (rowSums(params$counts >= params$minSingle) == 0)]
```
