...
params:
  ...
  widthsFile: NULL                    # Feature lengths.
  widthsCol: NULL                     # Feature lengths.
  createID: FALSE
...
my_pairwise_internal_corels <- function(mat, samples, method = "pearson", rds=NULL, txs=3, minMean=0, minSingle=0) {
  # mat <- tpm[topgenes,]; samples <- covars$Sample
...
do_pca <- function(countsmat, covars, scale = TRUE, center = TRUE, rds=NULL, topgenes=NULL, exclude=NULL, minMean=0, minSingle=0, ntop=10) {
  ...
  pvar1 <- ggplot() +
      geom_point(data=genevar[!selected & excluded, ], aes(x=Mean, y=StDev), shape=16, size=0.8, alpha=0.5, colour='grey70') +
      geom_point(data=genevar[!selected & !excluded, ],aes(x=Mean, y=StDev), shape=16, size=0.8, alpha=1, colour='black') +
      geom_point(data=genevar[(selected), ], aes(x=Mean, y=StDev), shape=16, size=1, alpha=1, colour='red') +
      ...
      labs(x="Mean (TPM or RPM)", y="Standard Deviation", title=paste(nrow(genevar[StDev>0, ]), "variable features"), 
...
message(paste("Covariates table:", params$covars))
...
```{r sample_info}
# Covariates
covars <- fread(params$covars, colClasses = 'character')
...
```{r input}
# Counts
counts <- fread(params$cts, colClasses="character")  # prevent any numeric IDs converting a string column to integer
...
counts <- as.matrix(counts[, covars$Sample, with=FALSE])    # Order and subset columns
counts <- matrix(as.numeric(counts), ncol=ncol(counts))
colnames(counts) <- covars$Sample  # because the type conversion lost them
...
Correlations and PCA will be based on the TPM values. If no feature lengths were provided, RPM values will be used instead.
...
widths <- all(is.null(params$widthsFile), is.null(params$widthsCol))
if (widths) {
...
setnames(TPM, c('id', paste0(colnames(tpm), ifelse(is.null(widths), '.RPM', '.TPM')) ))
fwrite(TPM, file=sub(".txt|.tsv", ".scaled.txt", params$cts), quote=FALSE, sep="\t")


# fwrite(data.table(id=rownames(counts), 
#                   mean=rowMeans(counts), max=rowMaxs(counts), signles=rowSums(counts>=params$minSingle), sd=rowSds(counts),
#                   sdt=rowSds(tpm), cvt=rowSds(tpm)/rowMeans(tpm)),
#        file="/Volumes/groups/busslinger/Kimon/test.txt", sep="\t", quote=FALSE)
```


# Filter

Genes with mean raw count across all samples below `r params$minMean` are excluded, unless any of the samples has raw count of at least `r params$minSingle`. Fr PCA all genes are given equal weight, but low expression genes are proportionally more variable, so filtering them in advance reduces noise.
...
message(paste(n - length(topgenes), "features eliminated, out of", n))

if (!is.null(params$specnorm)) {
  n <- length(topgenes)
  topgenes <- topgenes[! topgenes %in% rownames(tpm)[grepl(params$specnorm, rownames(tpm), ignore.case=TRUE, perl=TRUE)] ]
  message(paste("Additionally excluding", n - length(topgenes), "genes by name criteria."))
}

# Further reduce the above genes to the top most dispersed, as in DESeq2
cv <- sort(rowSds(tpm[topgenes, ]) / rowMeans(tpm[topgenes, ]), decreasing=TRUE)
...
exclude <- rownames(counts)[(rowMeans(counts) < params$minMean) & (rowSums(counts >= params$minSingle) == 0)]
```
...
The coefficients may disagree to some considerable extent, as a result of how observation density is distributed along the axes. SCC is more sensitive to value changes in the centre of the bell distribution where there are more measurements near one another, and less sensitive in the tails of the bell where there are fewer measurements and they are farther apart from one another.

## Count thresholds

Correlations based on all genes that meet the count thresholds.

```{r corr1, fig.height=6, fig.width=9}
corels1 <- my_pairwise_internal_corels(tpm[topgenes,], covars$Sample, method="pearson",
                                      minMean = 0, minSingle = 0,
                                      rds = file.path(params$RDSdir,
                                                      sub(".txt|.tsv", "_corrP.RDS", basename(params$cts))))
corels2 <- my_pairwise_internal_corels(tpm[topgenes, ], covars$Sample, method="spearman",
                                      minMean = 0, minSingle = 0,
                                      rds = file.path(params$RDSdir,
                                                      sub(".txt|.tsv", "_corrS.RDS", basename(params$cts))))

print( (corels1[['frnc']] + theme(legend.position="none")) | 
         (corels2[['frnc']] + theme(axis.text.y=element_blank())) )
```

```{r corr2, fig.height=6, fig.width=9}
print( corels1[['drc']] )
print( corels2[['drc']] )
```
...
Correlations using the top `r params$topVars` dispersed genes (out of those that already meet the count thresholds).

```{r corr1b, fig.height=6, fig.width=9}
corels1b <- my_pairwise_internal_corels(tpm[topgenes2, ], covars$Sample, method="pearson",
                                      minMean = 0, minSingle = 0,
                                      rds = file.path(params$RDSdir,
                                                      sub(".txt|.tsv", "_corrPtop.RDS", basename(params$cts))))
corels2b <- my_pairwise_internal_corels(tpm[topgenes2, ], covars$Sample, method="spearman",
                                      minMean = 0, minSingle = 0,
                                      rds = file.path(params$RDSdir,
                                                      sub(".txt|.tsv", "_corrStop.RDS", basename(params$cts))))

print( (corels1b[['frnc']] + theme(legend.position="none")) | 
         (corels2b[['frnc']] + theme(axis.text.y=element_blank())) )
```

```{r corr2b, fig.height=6, fig.width=9}
print( corels1b[['drc']] )
print( corels2b[['drc']] )
```
...
For this, the library-normalized expression (TPM or RPM) of the features is further standardized. This compensates for large differences in magnitude range and gives every feature equal weight. Features with exactly `0` variability have to be excluded for PCA, they are typically features that are not detected in any of the samples.
...
```{r pca}
all_pca <- do_pca(tpm, covars, ntop = params$ntop,
                  topgenes= topgenes, exclude=exclude,
                  minMean = 0, minSingle = 0, 
                  rds = file.path(params$RDSdir,
...
```{r pcab}
all_pcab <- do_pca(tpm, covars, ntop = params$ntop,
                  topgenes= topgenes2, exclude=exclude,
                  minMean = 0, minSingle = 0, 
                  rds = file.path(params$RDSdir,
                                  sub(".txt|.tsv", "_pcatop.RDS", basename(params$cts))))
```
...
```{r pcsb, fig.width=8, fig.height=12}
print( all_pcab$pc2d )
```
