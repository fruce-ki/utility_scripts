...
# Determine number of clusters by silhouette.
...
ddde <- lapply(coefficients, function(name) {
  ...
  # Rename genes
  if (!is.null(anno_df)) {         
    de[["feature"]] <- anno_df$gene_name[match(.$featureID, anno_df$gene_id)]
  } else {
...
print(
  ggplot(order_heat(hi1, mat), aes(x=Sample, y=Geneid, fill=Expression)) +
    facet_grid(cluster ~ ., scales= "free", drop = TRUE) +
    geom_tile() +
    scale_fill_gradientn(colours=c("darkblue", "steelblue", "white", "darkorange", "darkred")) +
...
print(
  ggplot(order_heat(hi2, mat), aes(x=Sample, y=Geneid, fill=Expression)) +
    facet_grid(cluster ~ ., scales= "free", drop = TRUE) +
    geom_tile() +
    scale_fill_gradientn(colours=c("darkblue", "steelblue", "white", "darkorange", "darkred")) +
...
print(
  ggplot(order_heat(hi3, mat), aes(x=Sample, y=Geneid, fill=Expression)) +
    facet_grid(cluster ~ ., scales= "free", drop = TRUE) +
    geom_tile() +
    scale_fill_gradientn(colours=c("darkblue", "steelblue", "white", "darkorange", "darkred")) +
