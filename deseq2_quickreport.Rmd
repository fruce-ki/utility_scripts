...
# Clustered row order. In principle each gene behaves differently, but may be redundant for vst-tranformed counts.
rhc <- hclust(dist(scale(mat, center=TRUE, scale=TRUE), method='euclidean'), method=clustmeth)
rord <- rownames(mat)[rhc$order]
...
# plot_dendro(as.dendrogram(rhc), xmin=-2, width= 900)
```
...
order_heat <- function(x, y) {
  ...
  rhc <- hclust(dist(scale(sm, center=TRUE, scale=TRUE), method='euclidean'), method=clustmeth)
