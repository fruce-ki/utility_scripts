...
ddde <- lapply(coefficients, function(name) {
  ...
  if (!is.null(anno_df)) {         
    de <- de %>%
      mutate(feature = anno_df$gene_name[match(.$featureID, anno_df$gene_id)])
  }
...
for(name in coefficients) {         # de <- ddde[1]
  ...
  print(head(detop$feature, n=ndetop), quote=)
...
# Clustered row order. Not centering on the mean results in more sensible visual groups. 
rhc <- hclust(dist(scale(mat, center=FALSE, scale=TRUE), method='euclidean'), method=clustmeth)
rord <- rownames(mat)[rhc$order]
# Clustered column order. Each sample conisted of the same genes, so they are directly comparable.
...
plot_dendro(as.dendrogram(rhc), xmin=-1, width= 800)
```
...
order_heat <- function(x, y) {
  ...
  rhc <- hclust(dist(scale(sm, center=FALSE, scale=TRUE), method='euclidean'), method=clustmeth)
...
print(
  ggplot(order_heat(hi1, mat), aes(x=Sample, y=Geneid, fill=Transformed_Count)) +
    geom_tile() +
    scale_fill_gradientn(colours=c("darkblue", "steelblue", "white", "darkorange", "darkred")) +
    ggtitle( paste('Top', length(hi1), 'high-loading genes for PC1') ) +
    theme(panel.background = element_rect(fill='white'),
          # axis.text.y = element_blank(),
          # axis.title.y = element_blank(),
          axis.ticks.y = element_blank(),
...
print(
  ggplot(order_heat(hi2, mat), aes(x=Sample, y=Geneid, fill=Transformed_Count)) +
    geom_tile() +
    scale_fill_gradientn(colours=c("darkblue", "steelblue", "white", "darkorange", "darkred")) +
    ggtitle( paste('Top', length(hi2), 'high-loading genes for PC2') ) +
    theme(panel.background = element_rect(fill='white'),
          # axis.text.y = element_blank(),
          # axis.title.y = element_blank(),
          axis.ticks.y = element_blank(),
...
print(
  ggplot(order_heat(hi3, mat), aes(x=Sample, y=Geneid, fill=Transformed_Count)) +
    geom_tile() +
    scale_fill_gradientn(colours=c("darkblue", "steelblue", "white", "darkorange", "darkred")) +
    ggtitle( paste('Top', length(hi3), 'high-loading genes for PC3') ) +
    theme(panel.background = element_rect(fill='white'),
          # axis.text.y = element_blank(),
          # axis.title.y = element_blank(),
          axis.ticks.y = element_blank(),
