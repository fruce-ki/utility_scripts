...
samples <- NULL
# Define sample aliases and order (disable clustering to preserve order)
...
Infer a suggestion for the number of clusters.
...
# Find optimal number of row clusters by silhouette.
...
cat( paste("Suggesteded number of row clusters:", copt) )
...
p1 <- ggplot(sil, aes(x=k, y=avg_sil)) +
  ...
  labs(title="Infering number of row clusters", x='Number of Groups', y='Average Silhouette Scores') + 
  theme_bw()
...
```{r}
if (!dontcluster){
  # Find optimal number of column clusters by silhouette.
  sil <- data.frame(k = 2:min(cmax, nrow(t(mat))-1),
                    avg_sil = vapply(2:min(cmax, nrow(t(mat))-1), function(k) { silhouette_score(k, t(mat)) }, double(1)) )
  copt2 <-sil$k[sil$avg_sil == max(sil$avg_sil)]
  cat( paste("Suggesteded number of column clusters:", copt2) )
  
  p1 <- ggplot(sil, aes(x=k, y=avg_sil)) +
    geom_point() +
    geom_line() +
    geom_vline(xintercept=copt2, linetype='dotted') +
    scale_x_continuous(breaks=seq(0,cmax,4), limits=c(0, cmax+1), expand=c(0,0)) +
    labs(title="Infering number of column clusters", x='Number of Groups', y='Average Silhouette Scores') + 
    theme_bw()
...
```{r}
# Override suggested number of clusters.
copt = 10   # 10 cellines/conditions at most I should expect 10 clusters of rows
# copt2 = 5   

# K-Means Clustering of rows
cat( paste("Using", copt, "row clusters.") )
...
if (!dontcluster){
  ...
  fit2 <- kmeans(t(mat), copt2, nstart=100)
  
  cat( paste("Using", copt2, "column clusters.") )
}
```
...
if (dontcluster){
  ...
  cord <- colnames(mat)[order(fit2$cluster)]
}
...
```{r fig.height=12, fig.width=10}
# mydata[grepl('_DT_', mydata$Sample), "Treatment"] <- "treated"
# mydata[!grepl('_DT_', mydata$Sample), "Treatment"] <- "untreated"
# mydata[grepl('_R_', mydata$Sample), "Resistance"] <- "resistant"
# mydata[!grepl('_R_', mydata$Sample), "Resistance"] <- "naive"
# mydata[grepl('Depl', mydata$Sample), "Cell"] <- "Depleted1"
# mydata[grepl('312A', mydata$Sample), "Cell"] <- "312A"
# mydata[grepl('314C', mydata$Sample), "Cell"] <- "314C"
# 
# print(
#   ggplot(mydata, aes(x=Sample, y=Geneid, fill=Expression)) +
#     facet_grid(cluster ~ paste(Treatment, Resistance), scales= "free", drop = TRUE) +
#     geom_tile() +
#     scale_fill_gradientn(colours=c("darkblue", "steelblue", "white", "darkorange", "darkred")) +
#     ggtitle( paste('Top', ntop, 'high-variance genes, split by condition') ) +
#     theme(panel.background = element_rect(fill='white'),
#           axis.text.y = element_blank(),
#           axis.title.y = element_blank(),
#           axis.ticks.y = element_blank(),
