...
DT3[, bracket := 0]
brackets <- sort(unique(c(1e9, 15e6, opt$cnt_thresh, 1e6, 0)), decreasing = TRUE)  # include large enough top limit and 0 for conditional to work correctly
for (i in 2:(length(brackets)) ) {
  DT3[unambiguousReads > brackets[i] & unambiguousReads <= brackets[i-1], bracket := brackets[i]]
}

DT3[, bracketPC := 0]
brackets <- sort(unique(c(100, 50, opt$pc_thresh, 5, 0)), decreasing = TRUE)  # include 100 and 0 for conditional to work correctly
for (i in 2:(length(brackets)) ) {
  DT3[unambiguousReadsPC > brackets[i] & unambiguousReadsPC <= brackets[i-1], bracketPC := brackets[i]]
}
