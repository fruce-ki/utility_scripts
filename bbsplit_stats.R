...
# opt <- list(baseDir = '/SCRATCH/sBatch3/', bbDir = 'outputs/bbsplit/', sampleSheet = 'samplesheet.rnaseq.csv')
...
fills <- c('darkgreen', 'orange3', 'black', rep('yellow3', times = length(levels(DT1$name)[!levels(DT1$name) %in% c('primary', 'ambiguous', 'ercc')])))
names(fills) <- c('primary', 'ambiguous', 'ercc', levels(DT1$name)[!levels(DT1$name) %in% c('primary', 'ambiguous', 'ercc')])
