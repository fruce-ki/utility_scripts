...
if (params$rlog) {
  vst <- rlog(deobj, blind = TRUE)
} else {
  vst <- vst(deobj, blind=TRUE)
}
vstmat <- assay(vst)
...
```{r exhist, fig.height=12, fig.width=12}
TPM <- melt(data.table(name=rownames(tpm), tpm), id.vars='name', variable.name='Sample', value.name='TPM')
ptpm <- ggplot(TPM, aes(x=TPM)) +
  facet_wrap(~ Sample) +
...
```{r correlations1, fig.height=15, fig.width=15}
...
```{r correlations2, fig.height=15, fig.width=18}
...
```{r correlations3, fig.height=15, fig.width=15}
...
```{r correlations4, fig.height=15, fig.width=18}
...
do_de_plots <- function(contrast, dds = deobj, rpms = tpm, regcnt=vstmat, ntop = params$ntop, addPrefix = params$longlabel,
  ...
  keep <- c("name", "FC", "padj",
              names(outtab)[grepl("refMean", names(outtab))], names(outtab)[grepl("treatMean", names(outtab))],
              "log2FoldChange", "mlog10p", "log2FoldChange.shrink", "baseMean", "maxCount", "maxScaledCount")
...
resall <- lapply(params$comparisons, function(contrast) {
  # contrast <-  params$comparisons[[1]]
  message('CONTRAST:', paste(contrast, collapse=' '))
