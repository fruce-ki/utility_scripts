...
# args <- c('~', 'test', '-:0:0', 'B18', '1', '/Volumes/groups/pavri/Kimon/ursi/mutPEseq/round5/process_paper/substractions/86745_B18_AIDER_s1_86736_B18_AIDKO_s1.point.stats', '/Volumes/groups/pavri/Kimon/ursi/mutPEseq/round5/process_paper/substractions/86747_B18_AIDER_s2_86737_B18_AIDKO_s1.point.stats', '/Volumes/groups/pavri/Kimon/ursi/mutPEseq/round5/process_paper/substractions/86756_B18_GCB1_86748_B18_G_e1.point.stats', '/Volumes/groups/pavri/Kimon/ursi/mutPEseq/round5/process_paper/substractions/86756_B18_GCB1_86758_B18_AIDKO_GCB1.point.stats')
...
pdf(file.path(outdir, paste0(prefix, '.pdf')), width=12, height=3*(length(statsfiles) + 1))
# mtxf <- file.path(outdir, paste0(prefix, '_matrices.txt'))
# cat('', file=mtxf)


# Base plot
pp <- ggplot(data.frame(x= 1:(showto - showfrom + 1), y=1), aes(x, y)) +
...
if (nrow(cdrs) > 0) {
  for (i in 1:nrow(cdrs)) {
    pplain <- pplain +
      geom_rect(xmin=cdrs[i,]$xleft, xmax=cdrs[i,]$xright, ymin=0, ymax=1, fill='grey95')
  }
}
if (nrow(warmspotshort) > 0) {
  pplain <- pplain +
    geom_segment(data=warmspotshort,
                 aes(x=xleft, xend=xright, y= 0.5, yend= 0.5),
                 inherit.aes=FALSE, colour='grey70', size=8)
}
if (nrow(hotspotshort) > 0) {
  pplain <- pplain +
    geom_segment(data=hotspotshort,
                 aes(x=xleft, xend=xright, y= 0.5, yend= 0.5),
                 inherit.aes=FALSE, colour='red1', size=8)
}
if (nrow(warmspotslong) > 0) {
  pplain <- pplain +
    geom_segment(data=warmspotslong,
                 aes(x=xleft, xend=xright, y= 0.8, yend= 0.8),
                 inherit.aes=FALSE, colour='grey50', size=8)
}
if (nrow(hotspotslong) > 0) {
  pplain <- pplain +
    geom_segment(data=hotspotslong,
                 aes(x=xleft, xend=xright, y= 0.8, yend= 0.8),
                 inherit.aes=FALSE, colour='red3', size=8)
}
if (nrow(coldspots) > 0) {
  pplain <- pplain +
    geom_segment(data=coldspots,
                 aes(x=xleft, xend=xright, y= 0.2, yend= 0.2),
                 inherit.aes=FALSE, colour='black', size=8)
}
...
for (sf in statsfiles){
  ...
  catcol <- rev(c("#440088", "#7700CC", "#AACCFF", "#AAAACC", "#0077FF", "#FFFFFF"))
  names(catcol) <- catlev
...
print(wrap_plots(rev(plist8), ncol=1))
