...
# Also, ECDC groups some of these territories together, whereas the map doesn't (except for the Virgin Islands, where it is the other way around and which is not worth the trouble to reconcile)/
...
```{r global, fig.height=10, fig.width=20}
...
ggsave(file.path(dirpath,'world.png'), plot = p3 / p4, device = png(),
       scale = 1, width = 12, height = 12, units = "cm", dpi = 300, limitsize = TRUE)
```

### Europe

```{r europe, fig.height=15, fig.width=18}
subDT <- merge(world_map[, .(region, long, lat, group, order)], 
...
ggsave(file.path(dirpath,'europe.png'), plot = p3 / p4, device = png(),
       scale = 1, width = 12, height = 12, units = "cm", dpi = 300, limitsize = TRUE)
```
...
ggsave(((p4|p3)/(p1|p2)), file=file.path(dirpath, 'EU_sub.png'), device='png')
```
