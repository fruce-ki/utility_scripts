...
# str(agecasenat)
# agecasenat[, date_week := as.Date(paste(year_week, '4'), "%Y-%V %w")]
##str(agecasesubnat)
# str(casedist)
...
# hosprate[, date_week := as.Date(paste(year_week, '4'), "%Y-%V %w")]
# str(movement)
# natcasedeath[, date_week := as.Date(paste(year_week, '4'), "%Y-%V %w")]
# str(subnatcaseday)
subnatcaseday[, date := as.Date(date)]
# str(subnatcaseweek)
subnatcaseweek[, year_week := sub('W', '', year_week)]
# subnatcaseweek[, date_week := as.Date(paste(year_week, '4'), "%Y-%V %w")]
# str(testing)
testing[, year_week := sub('W', '', year_week)]
# testing[, date_week := as.Date(paste(year_week, '4'), "%Y-%V %w")]
```
...
other_countries <- c('Switzerland', 'United Kingdom', 'USA', 'Colombia', 'Nicaragua', 'Egypt')
# holiday_options <- c('Croatia', 'Greece', 'Italy', 'Malta', 'Spain', 'Turkey', 'Egypt')
eu_countries <- c('Ireland', 'Portugal', 'Spain', 'France', 'Belgium', 'Luxembourg', 'Netherlands',
                  'Germany', 'Austria', 'Italy', 
                  'Croatia', 'Slovenia', 'Hungary','Slovakia', 'Czechia', 'Poland',
                  'Denmark', 'Sweden', 'Finland', 'Estonia', 'Latvia', 'Lithuania', 
                  'Romania', 'Bulgaria', 'Greece', 'Cyprus', 'Malta')
eu_countries <- factor(eu_countries, ordered=TRUE, labels=eu_countries)
...
setorder(natcasedeath, country, indicator, year_week)
natcasedeath[year_week %in% recentweeks, diff1 := .SD[.N, population_normalized] - shift(.SD[, population_normalized], type='lag', n=1), by=c('country', 'indicator')]
natcasedeath[year_week %in% recentweeks, diff2 := .SD[.N, population_normalized] - shift(.SD[, population_normalized], type='lag', n=2), by=c('country', 'indicator')]

setorder(subnatcaseweek, nuts_code, year_week)
subnatcaseweek[, diff1 := .SD[.N, rate_14_day_per_100k] - shift(.SD$rate_14_day_per_100k, n=1, type='lag'), by=c('nuts_code')]
subnatcaseweek[, diff2 := .SD[.N, rate_14_day_per_100k] - shift(.SD$rate_14_day_per_100k, n=2, type='lag'), by=c('nuts_code')]
```
...
```{r global, fig.height=13, fig.width=22}
...
p1 <- ggplot(subDT, 
  ...
  coord_fixed(ratio=1.2, ylim=c(-60, 80), xlim=c(-180,180)) + # Crop giant Antarctica and empty Arctic
  theme_void() +
...
subDT <- merge(world_map[, .(region, long, lat, group, order)], 
               natcasedeath[year_week==endweek & indicator=='cases',], 
               by.x='region', by.y='country', all.x=TRUE, all.y=FALSE, allow.cartesian=TRUE)

p5 <- ggplot(subDT, 
       aes(x=long, y=lat, group=group, fill=diff1)) +
  geom_polygon(colour='transparent', size=0.01) +
  scale_fill_gradient2(limits=c(min(subDT$diff1, subDT$diff2, na.rm=TRUE), max(subDT$diff1, subDT$diff2, na.rm=TRUE)), 
                       midpoint=0, low='forestgreen', mid='white', high='magenta', na.value='grey50', 
                       name=paste('weekly cases\nper', popnorm)) +
  coord_fixed(ratio=1.2, ylim=c(-60, 80), xlim=c(-180,180)) + # Crop giant Antarctica and empty Arctic
  labs(subtitle='Compared to the week before') +
  theme_void() +
  theme(panel.background=element_rect(fill=seablue),
        panel.grid=element_line(colour='grey80'),
        legend.position='left')
...
p6 <- ggplot(subDT, 
       aes(x=long, y=lat, group=group, fill=diff2)) +
  geom_polygon(colour='transparent', size=0.01) +
  scale_fill_gradient2(limits=c(min(subDT$diff1, subDT$diff2, na.rm=TRUE), max(subDT$diff1, subDT$diff2, na.rm=TRUE)), 
                       midpoint=0, low='forestgreen', mid='white', high='magenta', na.value='grey50', 
                       name=paste('weekly cases\nper', popnorm)) +
  coord_fixed(ratio=1.2, ylim=c(-60, 80), xlim=c(-180,180)) + # Crop giant Antarctica and empty Arctic
  labs(subtitle='Compared to 2 weeks before') +
  theme_void() +
  theme(panel.background=element_rect(fill=seablue),
        panel.grid=element_line(colour='grey80'),
        legend.position='right')
...
print( (p1 | p3) / (p2 | p4) / (p5 | p6) )

ggsave(file.path(dirpath,'world.png'), plot = ( (p1 | p3) / (p2 | p4) / (p5 | p6) ), device = png(),
       scale = 1, width = 22, height = 13, units = "in", dpi = 300, limitsize = TRUE)

```

### Europe

```{r europe, fig.height=15, fig.width=12}
subDT <- merge(world_map[, .(region, long, lat, group, order)], 
...
subDT <- merge(world_map[, .(region, long, lat, group, order)], 
               natcasedeath[year_week==endweek & indicator=='cases' & continent=='Europe',], 
               by.x='region', by.y='country', all.x=TRUE, all.y=FALSE, allow.cartesian=TRUE)
...
p5 <- ggplot(subDT, 
  ...
  geom_polygon(colour='transparent', size=0.01) +
  scale_fill_gradient2(limits=c(min(subDT$diff1, subDT$diff2, na.rm=TRUE), max(subDT$diff1, subDT$diff2, na.rm=TRUE)), 
                       midpoint=0, low='forestgreen', mid='white', high='magenta', na.value='transparent', 
                       name=paste('weekly cases\nper', popnorm)) +
  coord_map(projection='azequidistant',ylim=c(36, 68), xlim=c(-8,33)) +
  labs(subtitle='Compared to the week before') +
  theme_void() +
...
ggsave(file.path(dirpath,'europe.png'), plot = ( (p1 | p3) / (p2 | p4) / (p5 | p6) ), device = png(),
       scale = 1, width = 12, height = 15, units = "in", dpi = 300, limitsize = TRUE)
```
...
subDT <- merge(eut, 
               subnatcaseweek[year_week %in% recentweeks,],
               by.x='NUTS_ID', by.y='nuts_code', all.x=FALSE, all.y=FALSE)
setorder(subDT, year_week, LEVL_CODE, NUTS_ID, order)
...
p2 <- ggplot(subDT[year_week==endweek,],
  ...
  coord_map(projection='azequidistant', xlim=c(-8, 33), ylim=c(36,70)) +
  labs(title='14-day cases change', subtitle='Compared to the week before') +
  theme_void() +
  theme(legend.position='left',
        panel.background=element_rect(fill=seablue),
        panel.grid = element_line(colour='grey80'))
...
p3 <- ggplot(subDT[year_week==endweek,],
  ...
  scale_fill_gradient2(low='forestgreen', mid='white', high='magenta', name='per 100000') +
  coord_map(projection='azequidistant', xlim=c(-8, 33), ylim=c(36,70)) +
  labs(title='14-day cases change', subtitle='Compared to 2 weeks before') +
  theme_void() +
...
ggsave(( (p1 | p4) / (p2 | p3) ), file=file.path(dirpath, 'EU_sub.png'), device = png(),
       scale = 1, width = 11, height = 10, units = "in", dpi = 300, limitsize = TRUE)
```
...
subDT <- merge(eut, 
               subnatcaseweek[year_week %in% recentweeks & country=='Austria',],
               by.x='NUTS_ID', by.y='nuts_code', all.x=FALSE, all.y=FALSE)
setorder(subDT, LEVL_CODE, NUTS_ID, order)
...
p2 <- ggplot(subDT[year_week==endweek,],
       aes(x=long, y=lat, group=group,
           fill=diff1)) +
  geom_polygon(colour='transparent') +
  scale_fill_gradient2(low='forestgreen', mid='white', high='magenta',
                       limits=c(min(subDT$diff1, subDT$diff2), max(subDT$diff1, subDT$diff2)),
                       name='14-day cases\nper 100000' ) +
  coord_map(projection='azequidistant') +
  labs(subtitle='compared to the week before') +
  theme_void() +
...
ggsave(( (p1 | p4) / (p2 | p3) ), file=file.path(dirpath, 'Austria_sub.png'), device = png(),
       scale = 1, width = 14, height = 7, units = "in", dpi = 300, limitsize = TRUE)
```
...
ggsave(( (p1 | p4) / (p2 | p3) ), file=file.path(dirpath, 'Greece_sub.png'), device = png(),
       scale = 1, width = 14, height = 10, units = "in", dpi = 300, limitsize = TRUE)
```
...
ggsave(( (p1 | p4) / (p2 | p3) ), file=file.path(dirpath, 'Italy_sub.png'), device = png(),
       scale = 1, width = 14, height = 14, units = "in", dpi = 300, limitsize = TRUE)
```
...
```{r expo1, fig.width=10, fig.height=30}
# New vs Total
...
p <- expo(eu_countries)
print( p )
ggsave(p, file=file.path(dirpath, 'eu_expo.png'), device = png(),
     scale = 1, width = 10, height = 30, units = "in", dpi = 300, limitsize = TRUE)
```

```{r expo2, fig.width=10, fig.height=10}
p <- expo(c(homecountry, other_countries))
print( p )
ggsave(p, file=file.path(dirpath, 'other_expo.png'), device = png(),
     scale = 1, width = 10, height = 10, units = "in", dpi = 300, limitsize = TRUE)
```
...
```{r wave1, fig.width=10, fig.height=30}
...
p <- wave(eu_countries)
print( p )
ggsave(p, file=file.path(dirpath, 'eu_wave.png'), device='png',
     scale = 1, width = 10, height = 30, units = "in", dpi = 300, limitsize = TRUE)
```

```{r wave2, fig.width=10, fig.height=10}
p <- wave(c(homecountry, other_countries))
print( p )
ggsave(p, file=file.path(dirpath,'other_wave.png'), device='png',
     scale = 1, width = 10, height = 10, units = "in", dpi = 300, limitsize = TRUE)
```
...
```{r time1, fig.width=10, fig.height=30}
...
p <- tline(eu_countries)
print( p )
ggsave(p, file=file.path(dirpath, 'eu_tline.png'), device='png',
     scale = 1, width = 10, height = 30, units = "in", dpi = 300, limitsize = TRUE)
```

```{r time2, fig.width=10, fig.height=10}
p <- tline(c(homecountry, other_countries))
print( p )
ggsave(p, file=file.path(dirpath, 'other_tline.png'), device='png',
     scale = 1, width = 10, height = 10, units = "in", dpi = 300, limitsize = TRUE)
```
...
subtline <- function(selcnt='Austria'){
  p1 <- ggplot(subnatcaseweek[country==selcnt,],
    ...
    geom_path(size=1) +
    coord_cartesian(ylim=c(0,NA)) +
    labs(title=selcnt, y=paste('14-day cases per', popnorm)) +
    theme_bw() +
...
for (x in family_countries) {
  p <- subtline(x)
  print( p )
  ggsave(p, file=file.path(dirpath, paste(x, 'subtline.png', sep='_')), device='png',
       scale = 1, width = 15, height = 15, units = "in", dpi = 300, limitsize = TRUE)
}
```
