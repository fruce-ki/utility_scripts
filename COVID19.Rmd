...
other_countries <- c('United_Kingdom', 'Spain', 'France', 'Belgium', 'United_States_of_America', 'Sweden')
...
natcasedeath[country=='Virgin Islands', weekly_count := sum(weekly_count), by=year_week]
natcasedeath[country=='Virgin Islands', population := sum(population), by=year_week]
...
```{r global, fig.height=9, fig.width=20}
...
ggsave(file.path(dirpath,'world.png'), plot = ( (p1 | p3) / (p2 | p4) ), device = png(),
       scale = 1, width = 20, height = 9, units = "cm", dpi = 300, limitsize = TRUE)
```

### Europe

```{r europe, fig.height=9, fig.width=10}
subDT <- merge(world_map[, .(region, long, lat, group, order)], 
...
p1 <- ggplot(subDT, 
  ...
  coord_map(projection='azequidistant',ylim=c(36, 68), xlim=c(-10,35)) +
  theme_void() +
  theme(panel.background=element_rect(fill=seablue),
...
ggsave(file.path(dirpath,'europe.png'), plot = ( (p1 | p3) / (p2 | p4) ), device = png(),
       scale = 1, width = 10, height = 9, units = "cm", dpi = 300, limitsize = TRUE)
```

### EU

```{r eu, fig.height=10, fig.width=10}
subDT <- merge(eut, 
...
p1 <- ggplot(subDT,
  ...
  coord_map(projection='azequidistant', xlim=c(-5, 33), ylim=c(36,70)) +
...
subDT <- merge(eut, 
               subnatcaseweek[year_week %in% recentweeks[c(2,3)],],
               by.x='NUTS_ID', by.y='nuts_code', all.x=FALSE, all.y=FALSE)
subDT[, diff := rate_14_day_per_100k - shift(rate_14_day_per_100k, type='lag'), by=country]
setorder(subDT, LEVL_CODE, NUTS_ID, order)
...
p2 <- ggplot(subDT[year_week==endweek,],
  ...
  coord_map(projection='azequidistant', xlim=c(-5, 33), ylim=c(36,70)) +
  labs(title='weekly cases change', subtitle='from previous week') +
  theme_void() +
  theme(legend.position='left',
        panel.background=element_rect(fill=seablue),
        panel.grid = element_line(colour='grey90'))
...
p3 <- ggplot(subDT[year_week==endweek,],
  ...
  scale_fill_gradient2(low='forestgreen', mid='white', high='magenta', name='per 100000') +
  coord_map(projection='azequidistant', xlim=c(-5, 33), ylim=c(36,70)) +
  labs(title='weekly cases change', subtitle='from 2 weeks before') +
  theme_void() +
...
subDT <- merge(eut, 
               subnatcaseweek[year_week %in% recentweeks[c(2,3)] & country=='Austria',],
               by.x='NUTS_ID', by.y='nuts_code', all.x=FALSE, all.y=FALSE)
...
subDT <- merge(eut, 
               subnatcaseweek[year_week %in% recentweeks[c(2,3)] & country=='Greece',],
               by.x='NUTS_ID', by.y='nuts_code', all.x=FALSE, all.y=FALSE)
setorder(subDT, year_week, LEVL_CODE, NUTS_ID, order)
subDT[, diff := rate_14_day_per_100k - shift(rate_14_day_per_100k, type='lag'), by=country]
...
subDT <- merge(eut, 
               subnatcaseweek[year_week %in% recentweeks[c(2,3)] & country=='Italy',],
               by.x='NUTS_ID', by.y='nuts_code', all.x=FALSE, all.y=FALSE)
setorder(subDT, year_week, LEVL_CODE, NUTS_ID, order)
subDT[, diff := rate_14_day_per_100k - shift(rate_14_day_per_100k, type='lag'), by=country]
...
subDT <- merge(eut, 
               subnatcaseweek[year_week %in% recentweeks[c(2,3)] & country=='Germany',],
               by.x='NUTS_ID', by.y='nuts_code', all.x=FALSE, all.y=FALSE)
setorder(subDT, year_week, LEVL_CODE, NUTS_ID, order)
subDT[, diff := rate_14_day_per_100k - shift(rate_14_day_per_100k, type='lag'), by=country]
...
subDT <- natcasedeath[year_week==endweek & !is.na(weekly_count) & indicator=='death', 
                  .(country, weekly_count, population_normalized = weekly_count / population * popnorm)] [order(weekly_count, decreasing=TRUE), ] [!grepl('(total)', country),]
